# ============================================================================
# Phase 2 - 51 Videos (9 Superheroes + 30 Cartoon Characters + 12 Wild Cats)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase2.ps1
# ============================================================================
...
Write-Host "  PHASE 2: 9 Superheroes + 30 Cartoon Characters + 12 Wild Cats" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "superheroes\catalog.json"; Name = "Superheroes"; From = 21; To = 30 },
    @{ Path = "cartoon-characters\catalog.json"; Name = "Cartoon Characters"; From = 0; To = 30 },
    @{ Path = "wild-cats\catalog.json"; Name = "Wild Cats"; From = 0; To = 12 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=3200 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

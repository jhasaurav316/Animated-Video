# ============================================================================
# Phase 16 - 51 Videos (15 Snacks + 30 Drinks + 6 Candy)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase16.ps1
# ============================================================================
...
Write-Host "  PHASE 16: 15 Snacks + 30 Drinks + 6 Candy" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "snacks\catalog.json"; Name = "Snacks"; From = 15; To = 30 },
    @{ Path = "drinks\catalog.json"; Name = "Drinks"; From = 0; To = 30 },
    @{ Path = "candy\catalog.json"; Name = "Candy"; From = 0; To = 6 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=4600 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

# ============================================================================
# Phase 17 - 51 Videos (24 Candy + 27 Zoo Animals)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase17.ps1
# ============================================================================
...
Write-Host "  PHASE 17: 24 Candy + 27 Zoo Animals" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "candy\catalog.json"; Name = "Candy"; From = 6; To = 30 },
    @{ Path = "zoo-animals\catalog.json"; Name = "Zoo Animals"; From = 0; To = 27 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=4700 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

# ============================================================================
# Phase 12 - 51 Videos (9 Winter Words + 30 Summer Words + 12 Halloween)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase12.ps1
# ============================================================================
...
Write-Host "  PHASE 12: 9 Winter Words + 30 Summer Words + 12 Halloween" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "winter-words\catalog.json"; Name = "Winter Words"; From = 21; To = 30 },
    @{ Path = "summer-words\catalog.json"; Name = "Summer Words"; From = 0; To = 30 },
    @{ Path = "halloween\catalog.json"; Name = "Halloween"; From = 0; To = 12 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=4200 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

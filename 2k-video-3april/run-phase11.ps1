# ============================================================================
# Phase 11 - 51 Videos (30 Science Words + 21 Winter Words)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase11.ps1
# ============================================================================
...
Write-Host "  PHASE 11: 30 Science Words + 21 Winter Words" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "science-words\catalog.json"; Name = "Science Words"; From = 0; To = 30 },
    @{ Path = "winter-words\catalog.json"; Name = "Winter Words"; From = 0; To = 21 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=4100 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

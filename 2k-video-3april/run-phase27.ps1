# ============================================================================
# Phase 27 - 51 Videos (24 Tropical Fish + 27 Farm Crops)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase27.ps1
# ============================================================================
...
$catalogs = @(
    @{ Path = "tropical-fish\catalog.json"; Name = "Tropical Fish"; From = 6; To = 30 },
    @{ Path = "farm-crops\catalog.json"; Name = "Farm Crops"; From = 0; To = 27 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=5700 --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

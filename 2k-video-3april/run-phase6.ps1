# ============================================================================
# Phase 6 - 51 Videos (15 Numbers + 30 Colors + 6 Vehicles)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase6.ps1
# ============================================================================
...
Write-Host "  PHASE 6: 15 Numbers + 30 Colors + 6 Vehicles" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "numbers\catalog.json"; Name = "Numbers"; From = 15; To = 30 },
    @{ Path = "colors\catalog.json"; Name = "Colors"; From = 0; To = 30 },
    @{ Path = "vehicles\catalog.json"; Name = "Vehicles"; From = 0; To = 6 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=3600 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

# ============================================================================
# Phase 9 - 51 Videos (12 School Items + 30 Playground + 9 Musical Sounds)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase9.ps1
# ============================================================================
...
Write-Host "  PHASE 9: 12 School Items + 30 Playground + 9 Musical Sounds" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "school-items\catalog.json"; Name = "School Items"; From = 18; To = 30 },
    @{ Path = "playground\catalog.json"; Name = "Playground"; From = 0; To = 30 },
    @{ Path = "musical-sounds\catalog.json"; Name = "Musical Sounds"; From = 0; To = 9 }
)
...
Write-Host "  STEP 3: Rendering 51 Videos" -ForegroundColor Yellow
Write-Host ""
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=3900 --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

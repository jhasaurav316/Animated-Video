# ============================================================================
# Phase 22 - 51 Videos (9 Family Members + 30 Weather + 12 Planets)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase22.ps1
# ============================================================================
...
Write-Host "  PHASE 22: 9 Family Members + 30 Weather + 12 Planets" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "family-members\catalog.json"; Name = "Family Members"; From = 21; To = 30 },
    @{ Path = "weather\catalog.json"; Name = "Weather"; From = 0; To = 30 },
    @{ Path = "planets\catalog.json"; Name = "Planets"; From = 0; To = 12 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=5200 --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

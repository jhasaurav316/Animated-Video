# ============================================================================
# Phase 14 - 51 Videos (27 Garden Items + 24 Bath Time)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase14.ps1
# ============================================================================
...
Write-Host "  PHASE 14: 27 Garden Items + 24 Bath Time" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "garden-items\catalog.json"; Name = "Garden Items"; From = 3; To = 30 },
    @{ Path = "bath-time\catalog.json"; Name = "Bath Time"; From = 0; To = 24 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=4400 --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

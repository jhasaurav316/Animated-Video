# ============================================================================
# Phase 30 - 51 Videos (21 Cheese + 30 Dessert Types)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase30.ps1
# ============================================================================
...
Write-Host "  PHASE 30: 21 Cheese + 30 Dessert Types" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "cheese\catalog.json"; Name = "Cheese"; From = 9; To = 30 },
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=6000 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

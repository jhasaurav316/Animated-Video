# ============================================================================
# Phase 25 - 51 Videos (6 Rivers + 30 Languages + 15 Continents)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase25.ps1
# ============================================================================
...
Write-Host "  PHASE 25: 6 Rivers + 30 Languages + 15 Continents" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "rivers\catalog.json"; Name = "Rivers"; From = 24; To = 30 },
    @{ Path = "languages\catalog.json"; Name = "Languages"; From = 0; To = 30 },
    @{ Path = "continents\catalog.json"; Name = "Continents"; From = 0; To = 15 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=5500 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

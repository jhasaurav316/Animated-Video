# ============================================================================
# Phase 4 - 51 Videos (27 Jungle Animals + 24 Arctic Animals)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase4.ps1
# ============================================================================
...
Write-Host "  PHASE 4: 27 Jungle Animals + 24 Arctic Animals" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "jungle-animals\catalog.json"; Name = "Jungle Animals"; From = 3; To = 30 },
    @{ Path = "arctic-animals\catalog.json"; Name = "Arctic Animals"; From = 0; To = 24 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=3400 --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

# ============================================================================
# Phase 10 - 51 Videos (21 Musical Sounds + 30 Fairy Tales)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase10.ps1
# ============================================================================
...
Write-Host "  PHASE 10: 21 Musical Sounds + 30 Fairy Tales" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "musical-sounds\catalog.json"; Name = "Musical Sounds"; From = 9; To = 30 },
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=4000 --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

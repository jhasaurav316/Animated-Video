...
$bundleDir = Join-Path $ProjectDir "build-phase1"
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render --serve-url="$bundleDir" $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --port=3100
        $ErrorActionPreference = $prevEAP

# ============================================================================
# Phase 26 - 51 Videos (15 Continents + 30 Birds of Prey + 6 Tropical Fish)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase26.ps1
# ============================================================================
...
Write-Host "  PHASE 26: 15 Continents + 30 Birds of Prey + 6 Tropical Fish" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "continents\catalog.json"; Name = "Continents"; From = 15; To = 30 },
    @{ Path = "birds-of-prey\catalog.json"; Name = "Birds of Prey"; From = 0; To = 30 },
    @{ Path = "tropical-fish\catalog.json"; Name = "Tropical Fish"; From = 0; To = 6 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=5600 --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

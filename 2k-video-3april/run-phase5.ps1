# ============================================================================
# Phase 5 - 51 Videos (6 Arctic Animals + 30 Shapes + 15 Numbers)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase5.ps1
# ============================================================================
...
Write-Host "  PHASE 5: 6 Arctic Animals + 30 Shapes + 15 Numbers" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "arctic-animals\catalog.json"; Name = "Arctic Animals"; From = 24; To = 30 },
    @{ Path = "shapes\catalog.json"; Name = "Shapes"; From = 0; To = 30 },
    @{ Path = "numbers\catalog.json"; Name = "Numbers"; From = 0; To = 15 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=3500 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

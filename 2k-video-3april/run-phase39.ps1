# ============================================================================
# Phase 39 - 51 Videos (12 Airport + 30 Hospital + 9 Kitchen Appliances)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase39.ps1
# ============================================================================
...
Write-Host "  PHASE 39: 12 Airport + 30 Hospital + 9 Kitchen Appliances" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "airport\catalog.json"; Name = "Airport"; From = 18; To = 30 },
    @{ Path = "hospital\catalog.json"; Name = "Hospital"; From = 0; To = 30 },
    @{ Path = "kitchen-appliances\catalog.json"; Name = "Kitchen Appliances"; From = 0; To = 9 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=6900 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

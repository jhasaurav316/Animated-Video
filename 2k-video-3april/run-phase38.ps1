# ============================================================================
# Phase 38 - 51 Videos (3 Camping + 30 Beach + 18 Airport)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase38.ps1
# ============================================================================
...
Write-Host "  PHASE 38: 3 Camping + 30 Beach + 18 Airport" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "camping\catalog.json"; Name = "Camping"; From = 27; To = 30 },
    @{ Path = "beach\catalog.json"; Name = "Beach"; From = 0; To = 30 },
    @{ Path = "airport\catalog.json"; Name = "Airport"; From = 0; To = 18 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=6800 --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

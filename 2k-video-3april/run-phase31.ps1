# ============================================================================
# Phase 31 - 51 Videos (30 Dance Styles + 21 Martial Arts)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase31.ps1
# ============================================================================
...
Write-Host "  PHASE 31: 30 Dance Styles + 21 Martial Arts" -ForegroundColor Cyan
...
$catalogs = @(
    ...
    @{ Path = "martial-arts\catalog.json"; Name = "Martial Arts"; From = 0; To = 21 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=6100 --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

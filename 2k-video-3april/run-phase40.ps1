# ============================================================================
# Phase 40 - 51 Videos (21 Kitchen Appliances + 30 Cleaning Items)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase40.ps1
# ============================================================================
...
Write-Host "  PHASE 40: 21 Kitchen Appliances + 30 Cleaning Items" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "kitchen-appliances\catalog.json"; Name = "Kitchen Appliances"; From = 9; To = 30 },
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=7000 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

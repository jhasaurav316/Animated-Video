# ============================================================================
# Phase 18 - 51 Videos (3 Zoo Animals + 30 Underwater + 18 Rainforest)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase18.ps1
# ============================================================================
...
Write-Host "  PHASE 18: 3 Zoo Animals + 30 Underwater + 18 Rainforest" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "zoo-animals\catalog.json"; Name = "Zoo Animals"; From = 27; To = 30 },
    @{ Path = "underwater\catalog.json"; Name = "Underwater"; From = 0; To = 30 },
    @{ Path = "rainforest\catalog.json"; Name = "Rainforest"; From = 0; To = 18 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=4800 --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

# ============================================================================
# Phase 19 - 51 Videos (12 Rainforest + 30 Transportation + 9 Festivals)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase19.ps1
# ============================================================================
...
Write-Host "  PHASE 19: 12 Rainforest + 30 Transportation + 9 Festivals" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "rainforest\catalog.json"; Name = "Rainforest"; From = 18; To = 30 },
    @{ Path = "transportation\catalog.json"; Name = "Transportation"; From = 0; To = 30 },
    @{ Path = "festivals\catalog.json"; Name = "Festivals"; From = 0; To = 9 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=4900 --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

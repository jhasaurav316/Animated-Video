# ============================================================================
# Phase 34 - 51 Videos (27 Robots + 24 Dinosaur Types)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase34.ps1
# ============================================================================
...
Write-Host "  PHASE 34: 27 Robots + 24 Dinosaur Types" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "robots\catalog.json"; Name = "Robots"; From = 3; To = 30 },
    @{ Path = "dinosaur-types\catalog.json"; Name = "Dinosaur Types"; From = 0; To = 24 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=6400 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

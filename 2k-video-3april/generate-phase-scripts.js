...
Write-Host "  PRE-BUNDLE: Building once for all ${phaseTotal} videos..." -ForegroundColor Yellow
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=${3000 + phaseNum * 100} --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

# ============================================================================
# Phase 7 - 51 Videos (24 Vehicles + 27 Buildings)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase7.ps1
# ============================================================================
...
Write-Host "  PHASE 7: 24 Vehicles + 27 Buildings" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "vehicles\catalog.json"; Name = "Vehicles"; From = 6; To = 30 },
    @{ Path = "buildings\catalog.json"; Name = "Buildings"; From = 0; To = 27 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=3700 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

# ============================================================================
# Phase 32 - 51 Videos (9 Martial Arts + 30 Board Games + 12 Video Games)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase32.ps1
# ============================================================================
...
Write-Host "  PHASE 32: 9 Martial Arts + 30 Board Games + 12 Video Games" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "martial-arts\catalog.json"; Name = "Martial Arts"; From = 21; To = 30 },
    @{ Path = "board-games\catalog.json"; Name = "Board Games"; From = 0; To = 30 },
    @{ Path = "video-games\catalog.json"; Name = "Video Games"; From = 0; To = 12 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=6200 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

# ============================================================================
# Phase 36 - 51 Videos (15 Metals + 30 Fabrics + 6 Workshop Tools)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase36.ps1
# ============================================================================
...
Write-Host "  PHASE 36: 15 Metals + 30 Fabrics + 6 Workshop Tools" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "metals\catalog.json"; Name = "Metals"; From = 15; To = 30 },
    @{ Path = "fabrics\catalog.json"; Name = "Fabrics"; From = 0; To = 30 },
    @{ Path = "tools-workshop\catalog.json"; Name = "Workshop Tools"; From = 0; To = 6 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=6600 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

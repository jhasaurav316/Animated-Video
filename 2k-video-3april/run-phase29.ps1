# ============================================================================
# Phase 29 - 51 Videos (12 Breads + 30 Pasta + 9 Cheese)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase29.ps1
# ============================================================================
...
Write-Host "  PHASE 29: 12 Breads + 30 Pasta + 9 Cheese" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "breads\catalog.json"; Name = "Breads"; From = 18; To = 30 },
    @{ Path = "pasta\catalog.json"; Name = "Pasta"; From = 0; To = 30 },
    @{ Path = "cheese\catalog.json"; Name = "Cheese"; From = 0; To = 9 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=5900 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

# ============================================================================
# Phase 15 - 51 Videos (6 Bath Time + 30 Breakfast Foods + 15 Snacks)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase15.ps1
# ============================================================================
...
Write-Host "  PHASE 15: 6 Bath Time + 30 Breakfast Foods + 15 Snacks" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "bath-time\catalog.json"; Name = "Bath Time"; From = 24; To = 30 },
    @{ Path = "breakfast-foods\catalog.json"; Name = "Breakfast Foods"; From = 0; To = 30 },
    @{ Path = "snacks\catalog.json"; Name = "Snacks"; From = 0; To = 15 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=4500 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

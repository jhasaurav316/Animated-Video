# ============================================================================
# Phase 13 - 51 Videos (18 Halloween + 30 Christmas + 3 Garden Items)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase13.ps1
# ============================================================================
...
Write-Host "  PHASE 13: 18 Halloween + 30 Christmas + 3 Garden Items" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "halloween\catalog.json"; Name = "Halloween"; From = 12; To = 30 },
    @{ Path = "christmas\catalog.json"; Name = "Christmas"; From = 0; To = 30 },
    @{ Path = "garden-items\catalog.json"; Name = "Garden Items"; From = 0; To = 3 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=4300 --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

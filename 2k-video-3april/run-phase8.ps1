# ============================================================================
# Phase 8 - 51 Videos (3 Buildings + 30 Furniture + 18 School Items)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase8.ps1
# ============================================================================
...
Write-Host "  PHASE 8: 3 Buildings + 30 Furniture + 18 School Items" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "buildings\catalog.json"; Name = "Buildings"; From = 27; To = 30 },
    @{ Path = "furniture\catalog.json"; Name = "Furniture"; From = 0; To = 30 },
    @{ Path = "school-items\catalog.json"; Name = "School Items"; From = 0; To = 18 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=3800 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

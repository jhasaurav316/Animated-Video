# ============================================================================
# Phase 21 - 51 Videos (30 Baby Words + 21 Family Members)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase21.ps1
# ============================================================================
...
Write-Host "  PHASE 21: 30 Baby Words + 21 Family Members" -ForegroundColor Cyan
...
$catalogs = @(
    ...
    @{ Path = "family-members\catalog.json"; Name = "Family Members"; From = 0; To = 21 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=5100 --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

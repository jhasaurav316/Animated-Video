# ============================================================================
# Phase 3 - 51 Videos (18 Wild Cats + 30 Pet Animals + 3 Jungle Animals)
# ============================================================================
# Batch: 1k-video-3april | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase3.ps1
# ============================================================================
...
Write-Host "  PHASE 3: 18 Wild Cats + 30 Pet Animals + 3 Jungle Animals" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "wild-cats\catalog.json"; Name = "Wild Cats"; From = 12; To = 30 },
    @{ Path = "pet-animals\catalog.json"; Name = "Pet Animals"; From = 0; To = 30 },
    @{ Path = "jungle-animals\catalog.json"; Name = "Jungle Animals"; From = 0; To = 3 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=3300
        $ErrorActionPreference = $prevEAP

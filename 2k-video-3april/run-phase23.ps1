# ============================================================================
# Phase 23 - 51 Videos (18 Planets + 30 Oceans + 3 Mountains)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase23.ps1
# ============================================================================
...
Write-Host "  PHASE 23: 18 Planets + 30 Oceans + 3 Mountains" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "planets\catalog.json"; Name = "Planets"; From = 12; To = 30 },
    @{ Path = "oceans\catalog.json"; Name = "Oceans"; From = 0; To = 30 },
    @{ Path = "mountains\catalog.json"; Name = "Mountains"; From = 0; To = 3 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=5300 --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

# ============================================================================
# Phase 24 - 51 Videos (27 Mountains + 24 Rivers)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase24.ps1
# ============================================================================
...
Write-Host "  PHASE 24: 27 Mountains + 24 Rivers" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "mountains\catalog.json"; Name = "Mountains"; From = 3; To = 30 },
    @{ Path = "rivers\catalog.json"; Name = "Rivers"; From = 0; To = 24 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=5400 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

# ============================================================================
# Phase 28 - 51 Videos (3 Farm Crops + 30 Spices + 18 Breads)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase28.ps1
# ============================================================================
...
Write-Host "  PHASE 28: 3 Farm Crops + 30 Spices + 18 Breads" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "farm-crops\catalog.json"; Name = "Farm Crops"; From = 27; To = 30 },
    @{ Path = "spices\catalog.json"; Name = "Spices"; From = 0; To = 30 },
    @{ Path = "breads\catalog.json"; Name = "Breads"; From = 0; To = 18 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=5800 --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

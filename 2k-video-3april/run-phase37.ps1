# ============================================================================
# Phase 37 - 51 Videos (24 Workshop Tools + 27 Camping)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase37.ps1
# ============================================================================
...
Write-Host "  PHASE 37: 24 Workshop Tools + 27 Camping" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "tools-workshop\catalog.json"; Name = "Workshop Tools"; From = 6; To = 30 },
    @{ Path = "camping\catalog.json"; Name = "Camping"; From = 0; To = 27 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=6700 --bundle-cache=true
        $ErrorActionPreference = $prevEAP

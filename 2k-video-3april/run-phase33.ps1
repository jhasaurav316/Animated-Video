# ============================================================================
# Phase 33 - 51 Videos (18 Video Games + 30 Fairy Creatures + 3 Robots)
# ============================================================================
# Batch: 1k-video-3april (Batch 2) | Each video: 2:45 (165 seconds)
# Usage: cd C:\test\1k-video-3april && .\run-phase33.ps1
# ============================================================================
...
Write-Host "  PHASE 33: 18 Video Games + 30 Fairy Creatures + 3 Robots" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "video-games\catalog.json"; Name = "Video Games"; From = 12; To = 30 },
    @{ Path = "fairy-creatures\catalog.json"; Name = "Fairy Creatures"; From = 0; To = 30 },
    @{ Path = "robots\catalog.json"; Name = "Robots"; From = 0; To = 3 }
)
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        if ($bundleDir) {
            ...
            npx remotion render $compId "$outputFile" --concurrency=$cpuCores --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux --port=6300 --bundle-cache=true
        }
        $ErrorActionPreference = $prevEAP

#!/bin/bash
# ============================================================================
# Bulk Hindi Story Video Renderer
# ============================================================================
# Reads hindi-stories/catalog.json and renders every Hindi story as both
# landscape (YouTube) and shorts format using Remotion.
#
# Optimized for Standard_D4s_v3 (4 vCPUs, 16GB RAM).
# Skips already-rendered videos automatically.
#
# Usage:
#   bash hindi-stories/render-all.sh              # Render all
#   bash hindi-stories/render-all.sh --batch 1    # Day 1: stories 1-50
#   bash hindi-stories/render-all.sh --batch 2    # Day 2: stories 51-100
#   bash hindi-stories/render-all.sh --batch 3    # Day 3: stories 101-150
# ============================================================================
...
CATALOG="$PROJECT_DIR/hindi-stories/catalog.json"
OUT_YT="$PROJECT_DIR/out/story-youtube"
OUT_SHORTS="$PROJECT_DIR/out/story-shorts"
LOG_FILE="$PROJECT_DIR/hindi-stories/render-log.txt"
...
if [[ "${1:-}" == "--batch" ]] && [[ -n "${2:-}" ]]; then
  BATCH="$2"
  echo "Running BATCH $BATCH (stories $((( BATCH - 1 ) * BATCH_SIZE + 1))-$(( BATCH * BATCH_SIZE )))"
fi
...
# ---------- Setup ----------
echo "============================================"
echo "  Bulk Hindi Story Video Renderer"
echo "============================================"
echo ""
...
# Initialize log
echo "============================================" > "$LOG_FILE"
echo "  Hindi Story Render Log - Started at $(timestamp)" >> "$LOG_FILE"
echo "============================================" >> "$LOG_FILE"

# ---------- Parse Catalog ----------
# Extract story IDs from catalog (handles both array and object formats)
STORY_IDS=$(jq -r '
  if type == "array" then .[]
  elif .stories then .stories[]
  else empty
  end | .storyId // .id
' "$CATALOG")

TOTAL=$(echo "$STORY_IDS" | wc -l | tr -d ' ')
log "Found $TOTAL Hindi stories to render"
...
# Convert storyId to PascalCase composition ID with Story prefix (must match register-compositions.js)
to_pascal_case() {
...
for STORY_ID in $STORY_IDS; do
  ...
  # Batch filtering: skip stories outside the batch range
  if [[ -n "$BATCH" ]]; then
  ...
  COMP_ID="Story$(to_pascal_case "$STORY_ID")"

  log "[$CURRENT/$TOTAL] Processing: $STORY_ID (composition: $COMP_ID)"

  YT_FILE="$OUT_YT/${STORY_ID}.mp4"
  SHORTS_FILE="$OUT_SHORTS/${STORY_ID}-shorts.mp4"
  ...
  (
    ...
    if [ ! -f "$YT_FILE" ]; then
      if render_composition "$COMP_ID" "$YT_FILE" "$STORY_ID (landscape)"; then
    ...
    if [ ! -f "$SHORTS_FILE" ]; then
      if render_composition "${COMP_ID}Shorts" "$SHORTS_FILE" "$STORY_ID (shorts)"; then
...
echo ""
echo "============================================"
echo "  HINDI STORY RENDERING COMPLETE!"
echo "============================================"
echo ""

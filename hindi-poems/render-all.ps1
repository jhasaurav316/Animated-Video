# ============================================================================
# Bulk Hindi Poem Video Renderer (PowerShell)
# ============================================================================
# Reads hindi-poems/catalog.json and renders every Hindi rhyme as both
...
#
# Usage:
#   .\hindi-poems\render-all.ps1              # Render all
#   .\hindi-poems\render-all.ps1 -Batch 1     # Day 1: rhymes 1-50
#   .\hindi-poems\render-all.ps1 -Batch 2     # Day 2: rhymes 51-100
#   .\hindi-poems\render-all.ps1 -Batch 3     # Day 3: rhymes 101-150
# ============================================================================
...
$Catalog     = Join-Path $ProjectDir "hindi-poems\catalog.json"
$OutYT       = Join-Path $ProjectDir "out\hindi-youtube"
$OutShorts   = Join-Path $ProjectDir "out\hindi-shorts"
$LogFile     = Join-Path $ProjectDir "hindi-poems\render-log.txt"
$Concurrency = "100%"
$BatchSize   = 50
...
Write-Host ""
Write-Host "============================================"
Write-Host "  Bulk Hindi Poem Video Renderer (PS1)"
...
Add-Content -Path $LogFile -Value "  Hindi Poems Render Log - Started at $(Get-Timestamp)"
Add-Content -Path $LogFile -Value "============================================"

# ---------- Parse Catalog ----------
$catalogData = Get-Content -Raw $Catalog | ConvertFrom-Json

# Handle both array and object-with-rhymes formats
if ($catalogData -is [System.Array]) {
    $rhymes = $catalogData
} elseif ($catalogData.rhymes) {
    $rhymes = $catalogData.rhymes
} else {
...
$Total = $rhymes.Count
Write-Log "Found $Total Hindi poems to render"
...
if ($Batch -gt 0) {
    ...
    Write-Host ""
    Write-Log "BATCH MODE: Batch $Batch (poems $batchStart - $batchEnd)"
}
...
foreach ($rhyme in $rhymes) {
    ...
    $pascalName = ConvertTo-PascalCase $rhymeId
    $compId     = "Hindi$pascalName"

    # Elapsed time
    $elapsed    = (Get-Date) - $StartTime
    $elapsedStr = "{0:hh\:mm\:ss}" -f $elapsed

    Write-Log "[$Current/$Total] [$elapsedStr] Processing: $rhymeId (composition: $compId)"

    $ytFile     = Join-Path $OutYT "$rhymeId.mp4"
    $shortsFile = Join-Path $OutShorts "$rhymeId-shorts.mp4"
    ...
    $result = Render-Composition -CompId $compId -OutputFile $ytFile -Label "$rhymeId (landscape 1920x1080)"
    switch ($result) {
        "ok"      { $Rendered++ }
        "skipped" { $Skipped++ }
        "fail"    { $Failed++ }
    ...
    $result = Render-Composition -CompId "${compId}Shorts" -OutputFile $shortsFile -Label "$rhymeId (shorts 1080x1920)"
    switch ($result) {
        "ok"      { $Rendered++ }
        "skipped" { $Skipped++ }
        "fail"    { $Failed++ }
...
Write-Host ""
Write-Host "============================================"
Write-Host "  HINDI POEMS RENDERING COMPLETE!"
Write-Host "============================================"
Write-Host ""

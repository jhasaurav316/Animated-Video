#!/usr/bin/env node
// ============================================================================
// Auto-Register All Alphabet Shorts Compositions in AlphabetRoot.tsx
// ============================================================================
// Reads alphabet-shorts/catalog.json and generates a new AlphabetRoot.tsx that
// registers every alphabet video as a portrait (1080x1920) Remotion composition.
//
// Usage:
//   node alphabet-shorts/register-compositions.js
// ============================================================================
...
const CATALOG_PATH = path.join(PROJECT_ROOT, "alphabet-shorts", "catalog.json");
const ROOT_TSX_PATH = path.join(PROJECT_ROOT, "src", "AlphabetRoot.tsx");
const BACKUP_PATH = path.join(PROJECT_ROOT, "src", "AlphabetRoot.tsx.backup");

const FPS = 30;

// ---------- Helpers ----------

// Convert an id like "alphabet-fruits" to PascalCase "AlphabetFruits"
...
// Calculate duration in frames for an alphabet video
function calcDurationInFrames(video) {
  ...
  const letterCount = (video.letters || []).length;

  const totalSeconds = introDuration + letterCount * letterDuration + outroDuration;
...
function main() {
  console.log("============================================");
  console.log("  Register All Alphabet Shorts in AlphabetRoot.tsx");
  console.log("============================================\n");
  ...
  if (videos.length === 0) {
    console.error("ERROR: No alphabet videos found in catalog.");
    process.exit(1);
  }

  console.log(`Found ${videos.length} alphabet videos in catalog.`);

  // Back up existing AlphabetRoot.tsx
  if (fs.existsSync(ROOT_TSX_PATH)) {
    fs.copyFileSync(ROOT_TSX_PATH, BACKUP_PATH);
    console.log(`Backed up existing AlphabetRoot.tsx to AlphabetRoot.tsx.backup`);
  }

  // ---------- Generate AlphabetRoot.tsx content ----------
  let content = "";
  ...
  content += `export const AlphabetRemotionRoot: React.FC = () => {\n`;
  content += `  return (\n`;
  content += `    <>\n`;

  // Generate compositions (no folders needed - only 10 compositions)
  for (const video of videos) {
    const videoId = video.id;
    const compId = toPascalCase(videoId);
    const durationInFrames = calcDurationInFrames(video);

    // Build letters array string
    const lettersStr = JSON.stringify(video.letters || [], null, 10)
      .split("\n")
  ...
  console.log(`\nAlphabetRoot.tsx generated successfully!`);

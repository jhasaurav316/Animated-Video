import "./index.css";
import { Composition } from "remotion";
import { AlphabetShortsTemplate } from "./AlphabetShortsTemplate";
import type { AlphabetShortsProps } from "./AlphabetShortsTemplate";
import React from "react";

export const AbcAlphabetRemotionRoot: React.FC = () => {
  return (
    <>
      <Composition
        id="AbcAlphabetRainbow"
        component={AlphabetShortsTemplate}
        durationInFrames={2520}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "ABC Rainbow Alphabet",
          videoId: "alphabet-rainbow",
          bgGradient: ["#FF6B6B","#FFE66D"] as [string, string],
          accentColor: "#FF3366",
          letters: [
          {
          "letter": "A",
          "word": "Apple",
          "emoji": "🍎",
          "bgColor": "#FF4444"
          },
          {
          "letter": "B",
          "word": "Ball",
          "emoji": "⚽",
          "bgColor": "#FF8800"
          },
          {
          "letter": "C",
          "word": "Cat",
          "emoji": "🐱",
          "bgColor": "#FFCC00"
          },
          {
          "letter": "D",
          "word": "Dog",
          "emoji": "🐶",
          "bgColor": "#88CC00"
          },
          {
          "letter": "E",
          "word": "Elephant",
          "emoji": "🐘",
          "bgColor": "#00CC44"
          },
          {
          "letter": "F",
          "word": "Fish",
          "emoji": "🐟",
          "bgColor": "#00CCAA"
          },
          {
          "letter": "G",
          "word": "Grapes",
          "emoji": "🍇",
          "bgColor": "#0088CC"
          },
          {
          "letter": "H",
          "word": "Hat",
          "emoji": "🎩",
          "bgColor": "#0044CC"
          },
          {
          "letter": "I",
          "word": "Ice Cream",
          "emoji": "🍦",
          "bgColor": "#4400CC"
          },
          {
          "letter": "J",
          "word": "Juice",
          "emoji": "🧃",
          "bgColor": "#8800CC"
          },
          {
          "letter": "K",
          "word": "Kite",
          "emoji": "🪁",
          "bgColor": "#CC00AA"
          },
          {
          "letter": "L",
          "word": "Lion",
          "emoji": "🦁",
          "bgColor": "#CC0066"
          },
          {
          "letter": "M",
          "word": "Moon",
          "emoji": "🌙",
          "bgColor": "#CC0022"
          },
          {
          "letter": "N",
          "word": "Nest",
          "emoji": "🪺",
          "bgColor": "#FF3355"
          },
          {
          "letter": "O",
          "word": "Orange",
          "emoji": "🍊",
          "bgColor": "#FF6633"
          },
          {
          "letter": "P",
          "word": "Penguin",
          "emoji": "🐧",
          "bgColor": "#FF9911"
          },
          {
          "letter": "Q",
          "word": "Queen",
          "emoji": "👸",
          "bgColor": "#CCBB00"
          },
          {
          "letter": "R",
          "word": "Rainbow",
          "emoji": "🌈",
          "bgColor": "#66CC11"
          },
          {
          "letter": "S",
          "word": "Star",
          "emoji": "⭐",
          "bgColor": "#11CC66"
          },
          {
          "letter": "T",
          "word": "Tree",
          "emoji": "🌳",
          "bgColor": "#11CCBB"
          },
          {
          "letter": "U",
          "word": "Umbrella",
          "emoji": "☂️",
          "bgColor": "#1188CC"
          },
          {
          "letter": "V",
          "word": "Violin",
          "emoji": "🎻",
          "bgColor": "#3344CC"
          },
          {
          "letter": "W",
          "word": "Whale",
          "emoji": "🐋",
          "bgColor": "#6611CC"
          },
          {
          "letter": "X",
          "word": "Xylophone",
          "emoji": "🎵",
          "bgColor": "#AA11CC"
          },
          {
          "letter": "Y",
          "word": "Yacht",
          "emoji": "⛵",
          "bgColor": "#CC1188"
          },
          {
          "letter": "Z",
          "word": "Zebra",
          "emoji": "🦓",
          "bgColor": "#CC1144"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcAlphabetAdventure"
        component={AlphabetShortsTemplate}
        durationInFrames={2520}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "ABC Adventure Alphabet",
          videoId: "alphabet-adventure",
          bgGradient: ["#4CAF50","#81C784"] as [string, string],
          accentColor: "#2E7D32",
          letters: [
          {
          "letter": "A",
          "word": "Arrow",
          "emoji": "🏹",
          "bgColor": "#E53935"
          },
          {
          "letter": "B",
          "word": "Bridge",
          "emoji": "🌉",
          "bgColor": "#FF7043"
          },
          {
          "letter": "C",
          "word": "Castle",
          "emoji": "🏰",
          "bgColor": "#FFA726"
          },
          {
          "letter": "D",
          "word": "Dragon",
          "emoji": "🐉",
          "bgColor": "#FFCA28"
          },
          {
          "letter": "E",
          "word": "Explorer",
          "emoji": "🧭",
          "bgColor": "#D4E157"
          },
          {
          "letter": "F",
          "word": "Forest",
          "emoji": "🌲",
          "bgColor": "#66BB6A"
          },
          {
          "letter": "G",
          "word": "Galaxy",
          "emoji": "🌌",
          "bgColor": "#26A69A"
          },
          {
          "letter": "H",
          "word": "Hero",
          "emoji": "🦸",
          "bgColor": "#29B6F6"
          },
          {
          "letter": "I",
          "word": "Island",
          "emoji": "🏝️",
          "bgColor": "#42A5F5"
          },
          {
          "letter": "J",
          "word": "Jungle",
          "emoji": "🌴",
          "bgColor": "#5C6BC0"
          },
          {
          "letter": "K",
          "word": "Knight",
          "emoji": "⚔️",
          "bgColor": "#7E57C2"
          },
          {
          "letter": "L",
          "word": "Lava",
          "emoji": "🌋",
          "bgColor": "#AB47BC"
          },
          {
          "letter": "M",
          "word": "Mountain",
          "emoji": "🏔️",
          "bgColor": "#EC407A"
          },
          {
          "letter": "N",
          "word": "Navigator",
          "emoji": "🗺️",
          "bgColor": "#EF5350"
          },
          {
          "letter": "O",
          "word": "Ocean",
          "emoji": "🌊",
          "bgColor": "#FF7043"
          },
          {
          "letter": "P",
          "word": "Pirate",
          "emoji": "🏴‍☠️",
          "bgColor": "#FFA000"
          },
          {
          "letter": "Q",
          "word": "Quest",
          "emoji": "📜",
          "bgColor": "#FFD54F"
          },
          {
          "letter": "R",
          "word": "Rocket",
          "emoji": "🚀",
          "bgColor": "#AED581"
          },
          {
          "letter": "S",
          "word": "Sword",
          "emoji": "🗡️",
          "bgColor": "#4DB6AC"
          },
          {
          "letter": "T",
          "word": "Treasure",
          "emoji": "💎",
          "bgColor": "#4FC3F7"
          },
          {
          "letter": "U",
          "word": "Unicorn",
          "emoji": "🦄",
          "bgColor": "#7986CB"
          },
          {
          "letter": "V",
          "word": "Volcano",
          "emoji": "🌋",
          "bgColor": "#9575CD"
          },
          {
          "letter": "W",
          "word": "Warrior",
          "emoji": "🛡️",
          "bgColor": "#BA68C8"
          },
          {
          "letter": "X",
          "word": "X-ray",
          "emoji": "🔬",
          "bgColor": "#F06292"
          },
          {
          "letter": "Y",
          "word": "Yeti",
          "emoji": "❄️",
          "bgColor": "#E57373"
          },
          {
          "letter": "Z",
          "word": "Zeppelin",
          "emoji": "🎈",
          "bgColor": "#FF8A65"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcAlphabetMagical"
        component={AlphabetShortsTemplate}
        durationInFrames={2520}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "ABC Magical Alphabet",
          videoId: "alphabet-magical",
          bgGradient: ["#9C27B0","#CE93D8"] as [string, string],
          accentColor: "#6A1B9A",
          letters: [
          {
          "letter": "A",
          "word": "Angel",
          "emoji": "👼",
          "bgColor": "#F48FB1"
          },
          {
          "letter": "B",
          "word": "Butterfly",
          "emoji": "🦋",
          "bgColor": "#CE93D8"
          },
          {
          "letter": "C",
          "word": "Crown",
          "emoji": "👑",
          "bgColor": "#B39DDB"
          },
          {
          "letter": "D",
          "word": "Diamond",
          "emoji": "💎",
          "bgColor": "#9FA8DA"
          },
          {
          "letter": "E",
          "word": "Enchanted",
          "emoji": "✨",
          "bgColor": "#90CAF9"
          },
          {
          "letter": "F",
          "word": "Fairy",
          "emoji": "🧚",
          "bgColor": "#80DEEA"
          },
          {
          "letter": "G",
          "word": "Genie",
          "emoji": "🧞",
          "bgColor": "#80CBC4"
          },
          {
          "letter": "H",
          "word": "Heart",
          "emoji": "💖",
          "bgColor": "#A5D6A7"
          },
          {
          "letter": "I",
          "word": "Invisible",
          "emoji": "👻",
          "bgColor": "#C5E1A5"
          },
          {
          "letter": "J",
          "word": "Jewel",
          "emoji": "💍",
          "bgColor": "#E6EE9C"
          },
          {
          "letter": "K",
          "word": "Kingdom",
          "emoji": "🏰",
          "bgColor": "#FFF59D"
          },
          {
          "letter": "L",
          "word": "Lantern",
          "emoji": "🏮",
          "bgColor": "#FFE082"
          },
          {
          "letter": "M",
          "word": "Mermaid",
          "emoji": "🧜‍♀️",
          "bgColor": "#FFCC80"
          },
          {
          "letter": "N",
          "word": "Nymph",
          "emoji": "🧝‍♀️",
          "bgColor": "#FFAB91"
          },
          {
          "letter": "O",
          "word": "Oracle",
          "emoji": "🔮",
          "bgColor": "#EF9A9A"
          },
          {
          "letter": "P",
          "word": "Phoenix",
          "emoji": "🔥",
          "bgColor": "#F48FB1"
          },
          {
          "letter": "Q",
          "word": "Quartz",
          "emoji": "💠",
          "bgColor": "#CE93D8"
          },
          {
          "letter": "R",
          "word": "Rainbow",
          "emoji": "🌈",
          "bgColor": "#B39DDB"
          },
          {
          "letter": "S",
          "word": "Sparkle",
          "emoji": "💫",
          "bgColor": "#9FA8DA"
          },
          {
          "letter": "T",
          "word": "Tiara",
          "emoji": "👸",
          "bgColor": "#90CAF9"
          },
          {
          "letter": "U",
          "word": "Unicorn",
          "emoji": "🦄",
          "bgColor": "#80DEEA"
          },
          {
          "letter": "V",
          "word": "Velvet",
          "emoji": "🎀",
          "bgColor": "#80CBC4"
          },
          {
          "letter": "W",
          "word": "Wand",
          "emoji": "🪄",
          "bgColor": "#A5D6A7"
          },
          {
          "letter": "X",
          "word": "Xanadu",
          "emoji": "🌟",
          "bgColor": "#C5E1A5"
          },
          {
          "letter": "Y",
          "word": "Yarn",
          "emoji": "🧶",
          "bgColor": "#E6EE9C"
          },
          {
          "letter": "Z",
          "word": "Zenith",
          "emoji": "🌠",
          "bgColor": "#FFF59D"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcAlphabetNature"
        component={AlphabetShortsTemplate}
        durationInFrames={2520}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "ABC Nature Alphabet",
          videoId: "alphabet-nature",
          bgGradient: ["#00BCD4","#B2EBF2"] as [string, string],
          accentColor: "#006064",
          letters: [
          {
          "letter": "A",
          "word": "Acorn",
          "emoji": "🌰",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "B",
          "word": "Breeze",
          "emoji": "🌬️",
          "bgColor": "#78909C"
          },
          {
          "letter": "C",
          "word": "Cloud",
          "emoji": "☁️",
          "bgColor": "#90A4AE"
          },
          {
          "letter": "D",
          "word": "Dew",
          "emoji": "💧",
          "bgColor": "#4FC3F7"
          },
          {
          "letter": "E",
          "word": "Earth",
          "emoji": "🌍",
          "bgColor": "#4DB6AC"
          },
          {
          "letter": "F",
          "word": "Flower",
          "emoji": "🌸",
          "bgColor": "#F48FB1"
          },
          {
          "letter": "G",
          "word": "Grass",
          "emoji": "🌿",
          "bgColor": "#66BB6A"
          },
          {
          "letter": "H",
          "word": "Hill",
          "emoji": "⛰️",
          "bgColor": "#A1887F"
          },
          {
          "letter": "I",
          "word": "Ivy",
          "emoji": "🍀",
          "bgColor": "#81C784"
          },
          {
          "letter": "J",
          "word": "Jade",
          "emoji": "🪨",
          "bgColor": "#26A69A"
          },
          {
          "letter": "K",
          "word": "Kelp",
          "emoji": "🌊",
          "bgColor": "#009688"
          },
          {
          "letter": "L",
          "word": "Leaf",
          "emoji": "🍃",
          "bgColor": "#43A047"
          },
          {
          "letter": "M",
          "word": "Moss",
          "emoji": "🌱",
          "bgColor": "#2E7D32"
          },
          {
          "letter": "N",
          "word": "Nectar",
          "emoji": "🍯",
          "bgColor": "#FFB300"
          },
          {
          "letter": "O",
          "word": "Oasis",
          "emoji": "🏝️",
          "bgColor": "#00ACC1"
          },
          {
          "letter": "P",
          "word": "Petal",
          "emoji": "🌺",
          "bgColor": "#EC407A"
          },
          {
          "letter": "Q",
          "word": "Quartz",
          "emoji": "💎",
          "bgColor": "#AB47BC"
          },
          {
          "letter": "R",
          "word": "Rain",
          "emoji": "🌧️",
          "bgColor": "#5C6BC0"
          },
          {
          "letter": "S",
          "word": "Sun",
          "emoji": "☀️",
          "bgColor": "#FDD835"
          },
          {
          "letter": "T",
          "word": "Tulip",
          "emoji": "🌷",
          "bgColor": "#EF5350"
          },
          {
          "letter": "U",
          "word": "Undergrowth",
          "emoji": "🌿",
          "bgColor": "#388E3C"
          },
          {
          "letter": "V",
          "word": "Valley",
          "emoji": "🏞️",
          "bgColor": "#7CB342"
          },
          {
          "letter": "W",
          "word": "Waterfall",
          "emoji": "💦",
          "bgColor": "#039BE5"
          },
          {
          "letter": "X",
          "word": "Xerophyte",
          "emoji": "🌵",
          "bgColor": "#C0CA33"
          },
          {
          "letter": "Y",
          "word": "Yarrow",
          "emoji": "🌼",
          "bgColor": "#FFD600"
          },
          {
          "letter": "Z",
          "word": "Zephyr",
          "emoji": "🍃",
          "bgColor": "#80CBC4"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcAlphabetScience"
        component={AlphabetShortsTemplate}
        durationInFrames={2520}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "ABC Science Alphabet",
          videoId: "alphabet-science",
          bgGradient: ["#1565C0","#42A5F5"] as [string, string],
          accentColor: "#0D47A1",
          letters: [
          {
          "letter": "A",
          "word": "Atom",
          "emoji": "⚛️",
          "bgColor": "#1E88E5"
          },
          {
          "letter": "B",
          "word": "Beaker",
          "emoji": "🧪",
          "bgColor": "#43A047"
          },
          {
          "letter": "C",
          "word": "Circuit",
          "emoji": "🔌",
          "bgColor": "#FB8C00"
          },
          {
          "letter": "D",
          "word": "DNA",
          "emoji": "🧬",
          "bgColor": "#8E24AA"
          },
          {
          "letter": "E",
          "word": "Energy",
          "emoji": "⚡",
          "bgColor": "#FDD835"
          },
          {
          "letter": "F",
          "word": "Formula",
          "emoji": "📐",
          "bgColor": "#00ACC1"
          },
          {
          "letter": "G",
          "word": "Gravity",
          "emoji": "🌍",
          "bgColor": "#5E35B1"
          },
          {
          "letter": "H",
          "word": "Hydrogen",
          "emoji": "💨",
          "bgColor": "#00897B"
          },
          {
          "letter": "I",
          "word": "Isotope",
          "emoji": "🔬",
          "bgColor": "#3949AB"
          },
          {
          "letter": "J",
          "word": "Jupiter",
          "emoji": "🪐",
          "bgColor": "#D84315"
          },
          {
          "letter": "K",
          "word": "Kinetic",
          "emoji": "🏃",
          "bgColor": "#C62828"
          },
          {
          "letter": "L",
          "word": "Laser",
          "emoji": "🔴",
          "bgColor": "#AD1457"
          },
          {
          "letter": "M",
          "word": "Magnet",
          "emoji": "🧲",
          "bgColor": "#6A1B9A"
          },
          {
          "letter": "N",
          "word": "Neutron",
          "emoji": "⚛️",
          "bgColor": "#283593"
          },
          {
          "letter": "O",
          "word": "Orbit",
          "emoji": "🌏",
          "bgColor": "#0277BD"
          },
          {
          "letter": "P",
          "word": "Prism",
          "emoji": "🔺",
          "bgColor": "#00838F"
          },
          {
          "letter": "Q",
          "word": "Quantum",
          "emoji": "🌀",
          "bgColor": "#2E7D32"
          },
          {
          "letter": "R",
          "word": "Robot",
          "emoji": "🤖",
          "bgColor": "#558B2F"
          },
          {
          "letter": "S",
          "word": "Solar",
          "emoji": "☀️",
          "bgColor": "#F9A825"
          },
          {
          "letter": "T",
          "word": "Telescope",
          "emoji": "🔭",
          "bgColor": "#FF8F00"
          },
          {
          "letter": "U",
          "word": "Universe",
          "emoji": "🌌",
          "bgColor": "#EF6C00"
          },
          {
          "letter": "V",
          "word": "Voltage",
          "emoji": "🔋",
          "bgColor": "#D84315"
          },
          {
          "letter": "W",
          "word": "Wavelength",
          "emoji": "🌊",
          "bgColor": "#4E342E"
          },
          {
          "letter": "X",
          "word": "X-ray",
          "emoji": "🩻",
          "bgColor": "#37474F"
          },
          {
          "letter": "Y",
          "word": "Yttrium",
          "emoji": "🧪",
          "bgColor": "#546E7A"
          },
          {
          "letter": "Z",
          "word": "Zero",
          "emoji": "0️⃣",
          "bgColor": "#455A64"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcAnimalsSafari"
        component={AlphabetShortsTemplate}
        durationInFrames={2880}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "Safari Animals",
          videoId: "animals-safari",
          bgGradient: ["#FF8F00","#FFE082"] as [string, string],
          accentColor: "#E65100",
          letters: [
          {
          "letter": "A",
          "word": "Antelope",
          "emoji": "🦌",
          "bgColor": "#D4A056"
          },
          {
          "letter": "B",
          "word": "Buffalo",
          "emoji": "🐃",
          "bgColor": "#795548"
          },
          {
          "letter": "C",
          "word": "Cheetah",
          "emoji": "🐆",
          "bgColor": "#FFB300"
          },
          {
          "letter": "E",
          "word": "Elephant",
          "emoji": "🐘",
          "bgColor": "#78909C"
          },
          {
          "letter": "G",
          "word": "Giraffe",
          "emoji": "🦒",
          "bgColor": "#F9A825"
          },
          {
          "letter": "H",
          "word": "Hippo",
          "emoji": "🦛",
          "bgColor": "#7986CB"
          },
          {
          "letter": "H",
          "word": "Hyena",
          "emoji": "🐕",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "I",
          "word": "Impala",
          "emoji": "🦌",
          "bgColor": "#A1887F"
          },
          {
          "letter": "J",
          "word": "Jackal",
          "emoji": "🐺",
          "bgColor": "#BCAAA4"
          },
          {
          "letter": "K",
          "word": "Kudu",
          "emoji": "🦌",
          "bgColor": "#6D4C41"
          },
          {
          "letter": "L",
          "word": "Lion",
          "emoji": "🦁",
          "bgColor": "#FF8F00"
          },
          {
          "letter": "L",
          "word": "Leopard",
          "emoji": "🐆",
          "bgColor": "#FFB74D"
          },
          {
          "letter": "M",
          "word": "Meerkat",
          "emoji": "🦡",
          "bgColor": "#D7CCC8"
          },
          {
          "letter": "N",
          "word": "Nyala",
          "emoji": "🦌",
          "bgColor": "#5D4037"
          },
          {
          "letter": "O",
          "word": "Ostrich",
          "emoji": "🪶",
          "bgColor": "#E0E0E0"
          },
          {
          "letter": "P",
          "word": "Pangolin",
          "emoji": "🦔",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "Q",
          "word": "Quokka",
          "emoji": "🐹",
          "bgColor": "#A1887F"
          },
          {
          "letter": "R",
          "word": "Rhino",
          "emoji": "🦏",
          "bgColor": "#9E9E9E"
          },
          {
          "letter": "S",
          "word": "Springbok",
          "emoji": "🦌",
          "bgColor": "#FFCC02"
          },
          {
          "letter": "T",
          "word": "Tiger",
          "emoji": "🐅",
          "bgColor": "#FF6F00"
          },
          {
          "letter": "U",
          "word": "Umbrellabird",
          "emoji": "🐦",
          "bgColor": "#424242"
          },
          {
          "letter": "V",
          "word": "Vulture",
          "emoji": "🦅",
          "bgColor": "#616161"
          },
          {
          "letter": "W",
          "word": "Warthog",
          "emoji": "🐗",
          "bgColor": "#6D4C41"
          },
          {
          "letter": "X",
          "word": "Xerus",
          "emoji": "🐿️",
          "bgColor": "#BCAAA4"
          },
          {
          "letter": "Y",
          "word": "Yak",
          "emoji": "🐂",
          "bgColor": "#4E342E"
          },
          {
          "letter": "Z",
          "word": "Zebra",
          "emoji": "🦓",
          "bgColor": "#212121"
          },
          {
          "letter": "F",
          "word": "Flamingo",
          "emoji": "🦩",
          "bgColor": "#F06292"
          },
          {
          "letter": "G",
          "word": "Gorilla",
          "emoji": "🦍",
          "bgColor": "#455A64"
          },
          {
          "letter": "W",
          "word": "Wildebeest",
          "emoji": "🐃",
          "bgColor": "#5D4037"
          },
          {
          "letter": "B",
          "word": "Baboon",
          "emoji": "🐒",
          "bgColor": "#8D6E63"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcAnimalsOcean"
        component={AlphabetShortsTemplate}
        durationInFrames={2880}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "Ocean Animals",
          videoId: "animals-ocean",
          bgGradient: ["#0077B6","#00B4D8"] as [string, string],
          accentColor: "#023E8A",
          letters: [
          {
          "letter": "A",
          "word": "Angelfish",
          "emoji": "🐠",
          "bgColor": "#FFD54F"
          },
          {
          "letter": "B",
          "word": "Barracuda",
          "emoji": "🐟",
          "bgColor": "#90A4AE"
          },
          {
          "letter": "C",
          "word": "Clownfish",
          "emoji": "🐠",
          "bgColor": "#FF7043"
          },
          {
          "letter": "D",
          "word": "Dolphin",
          "emoji": "🐬",
          "bgColor": "#42A5F5"
          },
          {
          "letter": "E",
          "word": "Eel",
          "emoji": "🐍",
          "bgColor": "#5C6BC0"
          },
          {
          "letter": "F",
          "word": "Flounder",
          "emoji": "🐟",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "G",
          "word": "Grouper",
          "emoji": "🐟",
          "bgColor": "#66BB6A"
          },
          {
          "letter": "H",
          "word": "Hammerhead",
          "emoji": "🦈",
          "bgColor": "#78909C"
          },
          {
          "letter": "I",
          "word": "Iguana",
          "emoji": "🦎",
          "bgColor": "#66BB6A"
          },
          {
          "letter": "J",
          "word": "Jellyfish",
          "emoji": "🪼",
          "bgColor": "#CE93D8"
          },
          {
          "letter": "K",
          "word": "Killer Whale",
          "emoji": "🐋",
          "bgColor": "#37474F"
          },
          {
          "letter": "L",
          "word": "Lobster",
          "emoji": "🦞",
          "bgColor": "#E53935"
          },
          {
          "letter": "M",
          "word": "Manatee",
          "emoji": "🐋",
          "bgColor": "#78909C"
          },
          {
          "letter": "N",
          "word": "Narwhal",
          "emoji": "🐋",
          "bgColor": "#4FC3F7"
          },
          {
          "letter": "O",
          "word": "Octopus",
          "emoji": "🐙",
          "bgColor": "#AB47BC"
          },
          {
          "letter": "P",
          "word": "Pufferfish",
          "emoji": "🐡",
          "bgColor": "#FFD54F"
          },
          {
          "letter": "Q",
          "word": "Quahog",
          "emoji": "🐚",
          "bgColor": "#BCAAA4"
          },
          {
          "letter": "R",
          "word": "Ray",
          "emoji": "🐟",
          "bgColor": "#5C6BC0"
          },
          {
          "letter": "S",
          "word": "Seahorse",
          "emoji": "🐟",
          "bgColor": "#FF8A65"
          },
          {
          "letter": "T",
          "word": "Turtle",
          "emoji": "🐢",
          "bgColor": "#4CAF50"
          },
          {
          "letter": "U",
          "word": "Urchin",
          "emoji": "🦔",
          "bgColor": "#7E57C2"
          },
          {
          "letter": "V",
          "word": "Viper Fish",
          "emoji": "🐟",
          "bgColor": "#1A237E"
          },
          {
          "letter": "W",
          "word": "Walrus",
          "emoji": "🦭",
          "bgColor": "#6D4C41"
          },
          {
          "letter": "X",
          "word": "X-ray Fish",
          "emoji": "🐟",
          "bgColor": "#B0BEC5"
          },
          {
          "letter": "Y",
          "word": "Yellow Tang",
          "emoji": "🐠",
          "bgColor": "#FFEB3B"
          },
          {
          "letter": "Z",
          "word": "Zebrafish",
          "emoji": "🐟",
          "bgColor": "#455A64"
          },
          {
          "letter": "S",
          "word": "Starfish",
          "emoji": "⭐",
          "bgColor": "#FF7043"
          },
          {
          "letter": "C",
          "word": "Coral",
          "emoji": "🪸",
          "bgColor": "#F48FB1"
          },
          {
          "letter": "S",
          "word": "Shrimp",
          "emoji": "🦐",
          "bgColor": "#EF5350"
          },
          {
          "letter": "C",
          "word": "Crab",
          "emoji": "🦀",
          "bgColor": "#D32F2F"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcAnimalsPets"
        component={AlphabetShortsTemplate}
        durationInFrames={2880}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "Pet Animals",
          videoId: "animals-pets",
          bgGradient: ["#E91E63","#F48FB1"] as [string, string],
          accentColor: "#880E4F",
          letters: [
          {
          "letter": "A",
          "word": "Angora Cat",
          "emoji": "🐱",
          "bgColor": "#F5F5F5"
          },
          {
          "letter": "B",
          "word": "Beagle",
          "emoji": "🐶",
          "bgColor": "#A1887F"
          },
          {
          "letter": "C",
          "word": "Canary",
          "emoji": "🐤",
          "bgColor": "#FFEE58"
          },
          {
          "letter": "D",
          "word": "Dachshund",
          "emoji": "🐶",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "E",
          "word": "English Bulldog",
          "emoji": "🐕",
          "bgColor": "#BCAAA4"
          },
          {
          "letter": "F",
          "word": "Ferret",
          "emoji": "🦡",
          "bgColor": "#D7CCC8"
          },
          {
          "letter": "G",
          "word": "Golden Retriever",
          "emoji": "🐕",
          "bgColor": "#FFB300"
          },
          {
          "letter": "H",
          "word": "Hamster",
          "emoji": "🐹",
          "bgColor": "#FFCC80"
          },
          {
          "letter": "I",
          "word": "Iguana",
          "emoji": "🦎",
          "bgColor": "#66BB6A"
          },
          {
          "letter": "J",
          "word": "Japanese Chin",
          "emoji": "🐶",
          "bgColor": "#F5F5F5"
          },
          {
          "letter": "K",
          "word": "Kitten",
          "emoji": "🐱",
          "bgColor": "#FFE0B2"
          },
          {
          "letter": "L",
          "word": "Labrador",
          "emoji": "🐕",
          "bgColor": "#FFD54F"
          },
          {
          "letter": "M",
          "word": "Maine Coon",
          "emoji": "🐱",
          "bgColor": "#A1887F"
          },
          {
          "letter": "N",
          "word": "Newfoundland",
          "emoji": "🐕",
          "bgColor": "#4E342E"
          },
          {
          "letter": "O",
          "word": "Otter",
          "emoji": "🦦",
          "bgColor": "#6D4C41"
          },
          {
          "letter": "P",
          "word": "Poodle",
          "emoji": "🐩",
          "bgColor": "#F5F5F5"
          },
          {
          "letter": "Q",
          "word": "Quaker Parrot",
          "emoji": "🦜",
          "bgColor": "#66BB6A"
          },
          {
          "letter": "R",
          "word": "Rabbit",
          "emoji": "🐰",
          "bgColor": "#F48FB1"
          },
          {
          "letter": "S",
          "word": "Siamese",
          "emoji": "🐱",
          "bgColor": "#D7CCC8"
          },
          {
          "letter": "T",
          "word": "Tabby Cat",
          "emoji": "🐱",
          "bgColor": "#FF8A65"
          },
          {
          "letter": "U",
          "word": "Uromastyx",
          "emoji": "🦎",
          "bgColor": "#FFD54F"
          },
          {
          "letter": "V",
          "word": "Vizsla",
          "emoji": "🐕",
          "bgColor": "#D4A056"
          },
          {
          "letter": "W",
          "word": "Whippet",
          "emoji": "🐕",
          "bgColor": "#BDBDBD"
          },
          {
          "letter": "X",
          "word": "Xolo Dog",
          "emoji": "🐕",
          "bgColor": "#616161"
          },
          {
          "letter": "Y",
          "word": "Yorkshire",
          "emoji": "🐶",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "Z",
          "word": "Zebrafish",
          "emoji": "🐟",
          "bgColor": "#42A5F5"
          },
          {
          "letter": "P",
          "word": "Pug",
          "emoji": "🐶",
          "bgColor": "#BCAAA4"
          },
          {
          "letter": "P",
          "word": "Persian Cat",
          "emoji": "🐱",
          "bgColor": "#E0E0E0"
          },
          {
          "letter": "C",
          "word": "Cockatiel",
          "emoji": "🦜",
          "bgColor": "#FFCA28"
          },
          {
          "letter": "D",
          "word": "Dalmatian",
          "emoji": "🐕",
          "bgColor": "#F5F5F5"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcAnimalsJungle"
        component={AlphabetShortsTemplate}
        durationInFrames={2880}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "Jungle Animals",
          videoId: "animals-jungle",
          bgGradient: ["#1B5E20","#4CAF50"] as [string, string],
          accentColor: "#2E7D32",
          letters: [
          {
          "letter": "A",
          "word": "Anaconda",
          "emoji": "🐍",
          "bgColor": "#2E7D32"
          },
          {
          "letter": "B",
          "word": "Bat",
          "emoji": "🦇",
          "bgColor": "#37474F"
          },
          {
          "letter": "C",
          "word": "Chameleon",
          "emoji": "🦎",
          "bgColor": "#66BB6A"
          },
          {
          "letter": "D",
          "word": "Dart Frog",
          "emoji": "🐸",
          "bgColor": "#FF5722"
          },
          {
          "letter": "E",
          "word": "Eagle",
          "emoji": "🦅",
          "bgColor": "#795548"
          },
          {
          "letter": "F",
          "word": "Falcon",
          "emoji": "🦅",
          "bgColor": "#546E7A"
          },
          {
          "letter": "G",
          "word": "Gecko",
          "emoji": "🦎",
          "bgColor": "#8BC34A"
          },
          {
          "letter": "H",
          "word": "Hornbill",
          "emoji": "🐦",
          "bgColor": "#FF9800"
          },
          {
          "letter": "I",
          "word": "Ibis",
          "emoji": "🐦",
          "bgColor": "#F5F5F5"
          },
          {
          "letter": "J",
          "word": "Jaguar",
          "emoji": "🐆",
          "bgColor": "#FFB300"
          },
          {
          "letter": "K",
          "word": "Kingfisher",
          "emoji": "🐦",
          "bgColor": "#00BCD4"
          },
          {
          "letter": "L",
          "word": "Lemur",
          "emoji": "🐒",
          "bgColor": "#9E9E9E"
          },
          {
          "letter": "M",
          "word": "Macaw",
          "emoji": "🦜",
          "bgColor": "#F44336"
          },
          {
          "letter": "N",
          "word": "Newt",
          "emoji": "🦎",
          "bgColor": "#FF7043"
          },
          {
          "letter": "O",
          "word": "Ocelot",
          "emoji": "🐆",
          "bgColor": "#FFCA28"
          },
          {
          "letter": "P",
          "word": "Panther",
          "emoji": "🐈‍⬛",
          "bgColor": "#212121"
          },
          {
          "letter": "Q",
          "word": "Quetzal",
          "emoji": "🐦",
          "bgColor": "#00C853"
          },
          {
          "letter": "R",
          "word": "Red Panda",
          "emoji": "🐼",
          "bgColor": "#E64A19"
          },
          {
          "letter": "S",
          "word": "Sloth",
          "emoji": "🦥",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "T",
          "word": "Toucan",
          "emoji": "🐦",
          "bgColor": "#FF6F00"
          },
          {
          "letter": "U",
          "word": "Uakari",
          "emoji": "🐒",
          "bgColor": "#E53935"
          },
          {
          "letter": "V",
          "word": "Viper",
          "emoji": "🐍",
          "bgColor": "#4CAF50"
          },
          {
          "letter": "W",
          "word": "Wolf Spider",
          "emoji": "🕷️",
          "bgColor": "#5D4037"
          },
          {
          "letter": "X",
          "word": "Xenops",
          "emoji": "🐦",
          "bgColor": "#A1887F"
          },
          {
          "letter": "Y",
          "word": "Yellow Frog",
          "emoji": "🐸",
          "bgColor": "#FFEB3B"
          },
          {
          "letter": "Z",
          "word": "Zorilla",
          "emoji": "🦨",
          "bgColor": "#424242"
          },
          {
          "letter": "P",
          "word": "Python",
          "emoji": "🐍",
          "bgColor": "#33691E"
          },
          {
          "letter": "C",
          "word": "Capybara",
          "emoji": "🐹",
          "bgColor": "#795548"
          },
          {
          "letter": "T",
          "word": "Tapir",
          "emoji": "🐗",
          "bgColor": "#455A64"
          },
          {
          "letter": "P",
          "word": "Parrot",
          "emoji": "🦜",
          "bgColor": "#43A047"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcAnimalsFarm"
        component={AlphabetShortsTemplate}
        durationInFrames={2880}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "Farm Animals",
          videoId: "animals-farm",
          bgGradient: ["#795548","#BCAAA4"] as [string, string],
          accentColor: "#4E342E",
          letters: [
          {
          "letter": "A",
          "word": "Alpaca",
          "emoji": "🦙",
          "bgColor": "#F5F5F5"
          },
          {
          "letter": "B",
          "word": "Bull",
          "emoji": "🐂",
          "bgColor": "#5D4037"
          },
          {
          "letter": "C",
          "word": "Cow",
          "emoji": "🐄",
          "bgColor": "#F5F5F5"
          },
          {
          "letter": "D",
          "word": "Donkey",
          "emoji": "🫏",
          "bgColor": "#9E9E9E"
          },
          {
          "letter": "E",
          "word": "Ewe",
          "emoji": "🐑",
          "bgColor": "#E0E0E0"
          },
          {
          "letter": "F",
          "word": "Foal",
          "emoji": "🐴",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "G",
          "word": "Goat",
          "emoji": "🐐",
          "bgColor": "#BCAAA4"
          },
          {
          "letter": "H",
          "word": "Horse",
          "emoji": "🐎",
          "bgColor": "#6D4C41"
          },
          {
          "letter": "I",
          "word": "Indian Runner Duck",
          "emoji": "🦆",
          "bgColor": "#4CAF50"
          },
          {
          "letter": "J",
          "word": "Jersey Cow",
          "emoji": "🐄",
          "bgColor": "#A1887F"
          },
          {
          "letter": "K",
          "word": "Kid Goat",
          "emoji": "🐐",
          "bgColor": "#D7CCC8"
          },
          {
          "letter": "L",
          "word": "Lamb",
          "emoji": "🐑",
          "bgColor": "#F5F5F5"
          },
          {
          "letter": "M",
          "word": "Mare",
          "emoji": "🐴",
          "bgColor": "#795548"
          },
          {
          "letter": "N",
          "word": "Nanny Goat",
          "emoji": "🐐",
          "bgColor": "#BCAAA4"
          },
          {
          "letter": "O",
          "word": "Ox",
          "emoji": "🐂",
          "bgColor": "#5D4037"
          },
          {
          "letter": "P",
          "word": "Pig",
          "emoji": "🐷",
          "bgColor": "#F48FB1"
          },
          {
          "letter": "Q",
          "word": "Quail",
          "emoji": "🐦",
          "bgColor": "#A1887F"
          },
          {
          "letter": "R",
          "word": "Rooster",
          "emoji": "🐓",
          "bgColor": "#E53935"
          },
          {
          "letter": "S",
          "word": "Sheep",
          "emoji": "🐑",
          "bgColor": "#EEEEEE"
          },
          {
          "letter": "T",
          "word": "Turkey",
          "emoji": "🦃",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "U",
          "word": "Uppermill Hen",
          "emoji": "🐔",
          "bgColor": "#FF8A65"
          },
          {
          "letter": "V",
          "word": "Veal Calf",
          "emoji": "🐄",
          "bgColor": "#D7CCC8"
          },
          {
          "letter": "W",
          "word": "Welsh Pony",
          "emoji": "🐴",
          "bgColor": "#4E342E"
          },
          {
          "letter": "X",
          "word": "Xinjiang Horse",
          "emoji": "🐎",
          "bgColor": "#6D4C41"
          },
          {
          "letter": "Y",
          "word": "Yak",
          "emoji": "🐂",
          "bgColor": "#3E2723"
          },
          {
          "letter": "Z",
          "word": "Zebu",
          "emoji": "🐄",
          "bgColor": "#795548"
          },
          {
          "letter": "C",
          "word": "Chicken",
          "emoji": "🐔",
          "bgColor": "#FFCC80"
          },
          {
          "letter": "D",
          "word": "Duck",
          "emoji": "🦆",
          "bgColor": "#66BB6A"
          },
          {
          "letter": "G",
          "word": "Goose",
          "emoji": "🪿",
          "bgColor": "#E0E0E0"
          },
          {
          "letter": "H",
          "word": "Hen",
          "emoji": "🐔",
          "bgColor": "#A1887F"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcBirdsColorful"
        component={AlphabetShortsTemplate}
        durationInFrames={2880}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "Colorful Birds",
          videoId: "birds-colorful",
          bgGradient: ["#F44336","#FFCDD2"] as [string, string],
          accentColor: "#B71C1C",
          letters: [
          {
          "letter": "A",
          "word": "Albatross",
          "emoji": "🐦",
          "bgColor": "#ECEFF1"
          },
          {
          "letter": "B",
          "word": "Blue Jay",
          "emoji": "🐦",
          "bgColor": "#1E88E5"
          },
          {
          "letter": "C",
          "word": "Cardinal",
          "emoji": "🐦",
          "bgColor": "#E53935"
          },
          {
          "letter": "D",
          "word": "Dove",
          "emoji": "🕊️",
          "bgColor": "#EEEEEE"
          },
          {
          "letter": "E",
          "word": "Eagle",
          "emoji": "🦅",
          "bgColor": "#6D4C41"
          },
          {
          "letter": "F",
          "word": "Flamingo",
          "emoji": "🦩",
          "bgColor": "#F06292"
          },
          {
          "letter": "G",
          "word": "Goldfinch",
          "emoji": "🐦",
          "bgColor": "#FFD600"
          },
          {
          "letter": "H",
          "word": "Hummingbird",
          "emoji": "🐦",
          "bgColor": "#00C853"
          },
          {
          "letter": "I",
          "word": "Ibis",
          "emoji": "🐦",
          "bgColor": "#F5F5F5"
          },
          {
          "letter": "J",
          "word": "Jay",
          "emoji": "🐦",
          "bgColor": "#42A5F5"
          },
          {
          "letter": "K",
          "word": "Kiwi",
          "emoji": "🥝",
          "bgColor": "#6D4C41"
          },
          {
          "letter": "L",
          "word": "Lark",
          "emoji": "🐦",
          "bgColor": "#A1887F"
          },
          {
          "letter": "M",
          "word": "Macaw",
          "emoji": "🦜",
          "bgColor": "#F44336"
          },
          {
          "letter": "N",
          "word": "Nightingale",
          "emoji": "🐦",
          "bgColor": "#795548"
          },
          {
          "letter": "O",
          "word": "Oriole",
          "emoji": "🐦",
          "bgColor": "#FF9800"
          },
          {
          "letter": "P",
          "word": "Peacock",
          "emoji": "🦚",
          "bgColor": "#00695C"
          },
          {
          "letter": "Q",
          "word": "Quail",
          "emoji": "🐦",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "R",
          "word": "Robin",
          "emoji": "🐦",
          "bgColor": "#FF7043"
          },
          {
          "letter": "S",
          "word": "Sparrow",
          "emoji": "🐦",
          "bgColor": "#A1887F"
          },
          {
          "letter": "T",
          "word": "Toucan",
          "emoji": "🐦",
          "bgColor": "#FF6F00"
          },
          {
          "letter": "U",
          "word": "Umbrellabird",
          "emoji": "🐦",
          "bgColor": "#212121"
          },
          {
          "letter": "V",
          "word": "Vulture",
          "emoji": "🦅",
          "bgColor": "#616161"
          },
          {
          "letter": "W",
          "word": "Woodpecker",
          "emoji": "🐦",
          "bgColor": "#D32F2F"
          },
          {
          "letter": "X",
          "word": "Xenops",
          "emoji": "🐦",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "Y",
          "word": "Yellow Warbler",
          "emoji": "🐦",
          "bgColor": "#FFEB3B"
          },
          {
          "letter": "Z",
          "word": "Zebra Finch",
          "emoji": "🐦",
          "bgColor": "#FF7043"
          },
          {
          "letter": "K",
          "word": "Kingfisher",
          "emoji": "🐦",
          "bgColor": "#00BCD4"
          },
          {
          "letter": "P",
          "word": "Parakeet",
          "emoji": "🦜",
          "bgColor": "#4CAF50"
          },
          {
          "letter": "C",
          "word": "Cockatoo",
          "emoji": "🦜",
          "bgColor": "#FFF9C4"
          },
          {
          "letter": "P",
          "word": "Pelican",
          "emoji": "🐦",
          "bgColor": "#ECEFF1"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcBirdsTropical"
        component={AlphabetShortsTemplate}
        durationInFrames={2880}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "Tropical Birds",
          videoId: "birds-tropical",
          bgGradient: ["#00C853","#B9F6CA"] as [string, string],
          accentColor: "#1B5E20",
          letters: [
          {
          "letter": "A",
          "word": "Amazon Parrot",
          "emoji": "🦜",
          "bgColor": "#4CAF50"
          },
          {
          "letter": "B",
          "word": "Bird of Paradise",
          "emoji": "🐦",
          "bgColor": "#FF6F00"
          },
          {
          "letter": "C",
          "word": "Cockatiel",
          "emoji": "🦜",
          "bgColor": "#FFCA28"
          },
          {
          "letter": "D",
          "word": "Drongo",
          "emoji": "🐦",
          "bgColor": "#37474F"
          },
          {
          "letter": "E",
          "word": "Emerald Dove",
          "emoji": "🕊️",
          "bgColor": "#00C853"
          },
          {
          "letter": "F",
          "word": "Frigate Bird",
          "emoji": "🐦",
          "bgColor": "#212121"
          },
          {
          "letter": "G",
          "word": "Green Parrot",
          "emoji": "🦜",
          "bgColor": "#2E7D32"
          },
          {
          "letter": "H",
          "word": "Hoopoe",
          "emoji": "🐦",
          "bgColor": "#FF8A65"
          },
          {
          "letter": "I",
          "word": "Indian Roller",
          "emoji": "🐦",
          "bgColor": "#1E88E5"
          },
          {
          "letter": "J",
          "word": "Jacana",
          "emoji": "🐦",
          "bgColor": "#6D4C41"
          },
          {
          "letter": "K",
          "word": "Koel",
          "emoji": "🐦",
          "bgColor": "#424242"
          },
          {
          "letter": "L",
          "word": "Lorikeet",
          "emoji": "🦜",
          "bgColor": "#FF3D00"
          },
          {
          "letter": "M",
          "word": "Myna",
          "emoji": "🐦",
          "bgColor": "#5D4037"
          },
          {
          "letter": "N",
          "word": "Nuthatch",
          "emoji": "🐦",
          "bgColor": "#78909C"
          },
          {
          "letter": "O",
          "word": "Oropendola",
          "emoji": "🐦",
          "bgColor": "#FFD600"
          },
          {
          "letter": "P",
          "word": "Parrotlet",
          "emoji": "🦜",
          "bgColor": "#66BB6A"
          },
          {
          "letter": "Q",
          "word": "Quetzal",
          "emoji": "🐦",
          "bgColor": "#00E676"
          },
          {
          "letter": "R",
          "word": "Rosella",
          "emoji": "🦜",
          "bgColor": "#E53935"
          },
          {
          "letter": "S",
          "word": "Sunbird",
          "emoji": "🐦",
          "bgColor": "#FFD600"
          },
          {
          "letter": "T",
          "word": "Trogon",
          "emoji": "🐦",
          "bgColor": "#00C853"
          },
          {
          "letter": "U",
          "word": "Umbrella Cockatoo",
          "emoji": "🦜",
          "bgColor": "#FAFAFA"
          },
          {
          "letter": "V",
          "word": "Violet Sabrewing",
          "emoji": "🐦",
          "bgColor": "#7B1FA2"
          },
          {
          "letter": "W",
          "word": "Wren",
          "emoji": "🐦",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "X",
          "word": "Xenops",
          "emoji": "🐦",
          "bgColor": "#A1887F"
          },
          {
          "letter": "Y",
          "word": "Yellowhammer",
          "emoji": "🐦",
          "bgColor": "#FFEB3B"
          },
          {
          "letter": "Z",
          "word": "Zosterops",
          "emoji": "🐦",
          "bgColor": "#C0CA33"
          },
          {
          "letter": "T",
          "word": "Tanager",
          "emoji": "🐦",
          "bgColor": "#FF1744"
          },
          {
          "letter": "S",
          "word": "Starling",
          "emoji": "🐦",
          "bgColor": "#311B92"
          },
          {
          "letter": "B",
          "word": "Bulbul",
          "emoji": "🐦",
          "bgColor": "#795548"
          },
          {
          "letter": "C",
          "word": "Cuckoo",
          "emoji": "🐦",
          "bgColor": "#546E7A"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcBirdsFlying"
        component={AlphabetShortsTemplate}
        durationInFrames={2880}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "Birds of Flight",
          videoId: "birds-flying",
          bgGradient: ["#2196F3","#BBDEFB"] as [string, string],
          accentColor: "#0D47A1",
          letters: [
          {
          "letter": "A",
          "word": "Albatross",
          "emoji": "🐦",
          "bgColor": "#CFD8DC"
          },
          {
          "letter": "B",
          "word": "Bald Eagle",
          "emoji": "🦅",
          "bgColor": "#5D4037"
          },
          {
          "letter": "C",
          "word": "Condor",
          "emoji": "🦅",
          "bgColor": "#37474F"
          },
          {
          "letter": "D",
          "word": "Duck",
          "emoji": "🦆",
          "bgColor": "#4CAF50"
          },
          {
          "letter": "E",
          "word": "Egyptian Vulture",
          "emoji": "🦅",
          "bgColor": "#FFF9C4"
          },
          {
          "letter": "F",
          "word": "Falcon",
          "emoji": "🦅",
          "bgColor": "#546E7A"
          },
          {
          "letter": "G",
          "word": "Goose",
          "emoji": "🪿",
          "bgColor": "#E0E0E0"
          },
          {
          "letter": "H",
          "word": "Hawk",
          "emoji": "🦅",
          "bgColor": "#795548"
          },
          {
          "letter": "I",
          "word": "Imperial Eagle",
          "emoji": "🦅",
          "bgColor": "#3E2723"
          },
          {
          "letter": "J",
          "word": "Jabiru",
          "emoji": "🐦",
          "bgColor": "#ECEFF1"
          },
          {
          "letter": "K",
          "word": "Kestrel",
          "emoji": "🦅",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "L",
          "word": "Loon",
          "emoji": "🐦",
          "bgColor": "#37474F"
          },
          {
          "letter": "M",
          "word": "Merlin",
          "emoji": "🦅",
          "bgColor": "#455A64"
          },
          {
          "letter": "N",
          "word": "Northern Harrier",
          "emoji": "🦅",
          "bgColor": "#78909C"
          },
          {
          "letter": "O",
          "word": "Osprey",
          "emoji": "🦅",
          "bgColor": "#4E342E"
          },
          {
          "letter": "P",
          "word": "Peregrine",
          "emoji": "🦅",
          "bgColor": "#546E7A"
          },
          {
          "letter": "Q",
          "word": "Quail",
          "emoji": "🐦",
          "bgColor": "#A1887F"
          },
          {
          "letter": "R",
          "word": "Red Kite",
          "emoji": "🦅",
          "bgColor": "#BF360C"
          },
          {
          "letter": "S",
          "word": "Stork",
          "emoji": "🐦",
          "bgColor": "#F5F5F5"
          },
          {
          "letter": "T",
          "word": "Tern",
          "emoji": "🐦",
          "bgColor": "#ECEFF1"
          },
          {
          "letter": "U",
          "word": "Upland Sandpiper",
          "emoji": "🐦",
          "bgColor": "#A1887F"
          },
          {
          "letter": "V",
          "word": "Verdin",
          "emoji": "🐦",
          "bgColor": "#C0CA33"
          },
          {
          "letter": "W",
          "word": "Wagtail",
          "emoji": "🐦",
          "bgColor": "#FFD600"
          },
          {
          "letter": "X",
          "word": "Xantus",
          "emoji": "🐦",
          "bgColor": "#6D4C41"
          },
          {
          "letter": "Y",
          "word": "Yellowlegs",
          "emoji": "🐦",
          "bgColor": "#FFEB3B"
          },
          {
          "letter": "Z",
          "word": "Zitting Cisticola",
          "emoji": "🐦",
          "bgColor": "#A1887F"
          },
          {
          "letter": "S",
          "word": "Swift",
          "emoji": "🐦",
          "bgColor": "#263238"
          },
          {
          "letter": "C",
          "word": "Crane",
          "emoji": "🐦",
          "bgColor": "#BDBDBD"
          },
          {
          "letter": "H",
          "word": "Heron",
          "emoji": "🐦",
          "bgColor": "#607D8B"
          },
          {
          "letter": "P",
          "word": "Pelican",
          "emoji": "🐦",
          "bgColor": "#F5F5F5"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcBirdsGarden"
        component={AlphabetShortsTemplate}
        durationInFrames={2880}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "Garden Birds",
          videoId: "birds-garden",
          bgGradient: ["#8BC34A","#DCEDC8"] as [string, string],
          accentColor: "#33691E",
          letters: [
          {
          "letter": "A",
          "word": "American Robin",
          "emoji": "🐦",
          "bgColor": "#FF7043"
          },
          {
          "letter": "B",
          "word": "Bluebird",
          "emoji": "🐦",
          "bgColor": "#42A5F5"
          },
          {
          "letter": "C",
          "word": "Chickadee",
          "emoji": "🐦",
          "bgColor": "#E0E0E0"
          },
          {
          "letter": "D",
          "word": "Dove",
          "emoji": "🕊️",
          "bgColor": "#F5F5F5"
          },
          {
          "letter": "E",
          "word": "Eastern Bluebird",
          "emoji": "🐦",
          "bgColor": "#1E88E5"
          },
          {
          "letter": "F",
          "word": "Finch",
          "emoji": "🐦",
          "bgColor": "#FFCA28"
          },
          {
          "letter": "G",
          "word": "Grackle",
          "emoji": "🐦",
          "bgColor": "#311B92"
          },
          {
          "letter": "H",
          "word": "House Sparrow",
          "emoji": "🐦",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "I",
          "word": "Indigo Bunting",
          "emoji": "🐦",
          "bgColor": "#1565C0"
          },
          {
          "letter": "J",
          "word": "Junco",
          "emoji": "🐦",
          "bgColor": "#546E7A"
          },
          {
          "letter": "K",
          "word": "Killdeer",
          "emoji": "🐦",
          "bgColor": "#A1887F"
          },
          {
          "letter": "L",
          "word": "Linnet",
          "emoji": "🐦",
          "bgColor": "#EF5350"
          },
          {
          "letter": "M",
          "word": "Mockingbird",
          "emoji": "🐦",
          "bgColor": "#9E9E9E"
          },
          {
          "letter": "N",
          "word": "Nuthatch",
          "emoji": "🐦",
          "bgColor": "#78909C"
          },
          {
          "letter": "O",
          "word": "Oriole",
          "emoji": "🐦",
          "bgColor": "#FF9800"
          },
          {
          "letter": "P",
          "word": "Pigeon",
          "emoji": "🐦",
          "bgColor": "#78909C"
          },
          {
          "letter": "Q",
          "word": "Quail",
          "emoji": "🐦",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "R",
          "word": "Redstart",
          "emoji": "🐦",
          "bgColor": "#E53935"
          },
          {
          "letter": "S",
          "word": "Swallow",
          "emoji": "🐦",
          "bgColor": "#1565C0"
          },
          {
          "letter": "T",
          "word": "Thrasher",
          "emoji": "🐦",
          "bgColor": "#6D4C41"
          },
          {
          "letter": "U",
          "word": "Ural Owl",
          "emoji": "🦉",
          "bgColor": "#A1887F"
          },
          {
          "letter": "V",
          "word": "Vireo",
          "emoji": "🐦",
          "bgColor": "#7CB342"
          },
          {
          "letter": "W",
          "word": "Waxwing",
          "emoji": "🐦",
          "bgColor": "#FFCA28"
          },
          {
          "letter": "X",
          "word": "Xenops",
          "emoji": "🐦",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "Y",
          "word": "Yellowthroat",
          "emoji": "🐦",
          "bgColor": "#FFEB3B"
          },
          {
          "letter": "Z",
          "word": "Zenaida Dove",
          "emoji": "🕊️",
          "bgColor": "#D7CCC8"
          },
          {
          "letter": "T",
          "word": "Titmouse",
          "emoji": "🐦",
          "bgColor": "#BDBDBD"
          },
          {
          "letter": "W",
          "word": "Wren",
          "emoji": "🐦",
          "bgColor": "#795548"
          },
          {
          "letter": "S",
          "word": "Starling",
          "emoji": "🐦",
          "bgColor": "#263238"
          },
          {
          "letter": "C",
          "word": "Catbird",
          "emoji": "🐦",
          "bgColor": "#546E7A"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcBirdsNight"
        component={AlphabetShortsTemplate}
        durationInFrames={2880}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "Night Birds",
          videoId: "birds-night",
          bgGradient: ["#1A237E","#7986CB"] as [string, string],
          accentColor: "#0D47A1",
          letters: [
          {
          "letter": "A",
          "word": "Australian Owlet",
          "emoji": "🦉",
          "bgColor": "#5D4037"
          },
          {
          "letter": "B",
          "word": "Barn Owl",
          "emoji": "🦉",
          "bgColor": "#BCAAA4"
          },
          {
          "letter": "C",
          "word": "Chuck-will's-widow",
          "emoji": "🐦",
          "bgColor": "#6D4C41"
          },
          {
          "letter": "D",
          "word": "Dark Owl",
          "emoji": "🦉",
          "bgColor": "#212121"
          },
          {
          "letter": "E",
          "word": "Elf Owl",
          "emoji": "🦉",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "F",
          "word": "Frogmouth",
          "emoji": "🐦",
          "bgColor": "#795548"
          },
          {
          "letter": "G",
          "word": "Great Horned Owl",
          "emoji": "🦉",
          "bgColor": "#4E342E"
          },
          {
          "letter": "H",
          "word": "Hobby",
          "emoji": "🐦",
          "bgColor": "#455A64"
          },
          {
          "letter": "I",
          "word": "Indian Scops Owl",
          "emoji": "🦉",
          "bgColor": "#6D4C41"
          },
          {
          "letter": "J",
          "word": "Japanese Night Heron",
          "emoji": "🐦",
          "bgColor": "#37474F"
          },
          {
          "letter": "K",
          "word": "Kakapo",
          "emoji": "🦜",
          "bgColor": "#558B2F"
          },
          {
          "letter": "L",
          "word": "Long-eared Owl",
          "emoji": "🦉",
          "bgColor": "#5D4037"
          },
          {
          "letter": "M",
          "word": "Morepork",
          "emoji": "🦉",
          "bgColor": "#3E2723"
          },
          {
          "letter": "N",
          "word": "Nightjar",
          "emoji": "🐦",
          "bgColor": "#424242"
          },
          {
          "letter": "O",
          "word": "Oilbird",
          "emoji": "🐦",
          "bgColor": "#4E342E"
          },
          {
          "letter": "P",
          "word": "Potoo",
          "emoji": "🐦",
          "bgColor": "#795548"
          },
          {
          "letter": "Q",
          "word": "Quail Plover",
          "emoji": "🐦",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "R",
          "word": "Razorbill",
          "emoji": "🐦",
          "bgColor": "#263238"
          },
          {
          "letter": "S",
          "word": "Snowy Owl",
          "emoji": "🦉",
          "bgColor": "#ECEFF1"
          },
          {
          "letter": "T",
          "word": "Tawny Owl",
          "emoji": "🦉",
          "bgColor": "#6D4C41"
          },
          {
          "letter": "U",
          "word": "Ural Owl",
          "emoji": "🦉",
          "bgColor": "#A1887F"
          },
          {
          "letter": "V",
          "word": "Vermiculated Screech",
          "emoji": "🦉",
          "bgColor": "#5D4037"
          },
          {
          "letter": "W",
          "word": "Whippoorwill",
          "emoji": "🐦",
          "bgColor": "#546E7A"
          },
          {
          "letter": "X",
          "word": "Xenops",
          "emoji": "🐦",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "Y",
          "word": "Yellow Night Heron",
          "emoji": "🐦",
          "bgColor": "#F9A825"
          },
          {
          "letter": "Z",
          "word": "Zimmer's Tapaculo",
          "emoji": "🐦",
          "bgColor": "#455A64"
          },
          {
          "letter": "E",
          "word": "Eagle Owl",
          "emoji": "🦉",
          "bgColor": "#3E2723"
          },
          {
          "letter": "S",
          "word": "Screech Owl",
          "emoji": "🦉",
          "bgColor": "#4E342E"
          },
          {
          "letter": "B",
          "word": "Boobook",
          "emoji": "🦉",
          "bgColor": "#795548"
          },
          {
          "letter": "S",
          "word": "Short-eared Owl",
          "emoji": "🦉",
          "bgColor": "#BCAAA4"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcFruitsTropical"
        component={AlphabetShortsTemplate}
        durationInFrames={2880}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "Tropical Fruits",
          videoId: "fruits-tropical",
          bgGradient: ["#FF6F00","#FFCA28"] as [string, string],
          accentColor: "#E65100",
          letters: [
          {
          "letter": "A",
          "word": "Acai",
          "emoji": "🫐",
          "bgColor": "#4A148C"
          },
          {
          "letter": "B",
          "word": "Banana",
          "emoji": "🍌",
          "bgColor": "#FFD600"
          },
          {
          "letter": "C",
          "word": "Coconut",
          "emoji": "🥥",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "D",
          "word": "Dragonfruit",
          "emoji": "🐉",
          "bgColor": "#E91E63"
          },
          {
          "letter": "E",
          "word": "Elderberry",
          "emoji": "🫐",
          "bgColor": "#4A148C"
          },
          {
          "letter": "F",
          "word": "Fig",
          "emoji": "🍇",
          "bgColor": "#6A1B9A"
          },
          {
          "letter": "G",
          "word": "Guava",
          "emoji": "🍈",
          "bgColor": "#7CB342"
          },
          {
          "letter": "H",
          "word": "Honeydew",
          "emoji": "🍈",
          "bgColor": "#C5E1A5"
          },
          {
          "letter": "I",
          "word": "Indian Fig",
          "emoji": "🌵",
          "bgColor": "#66BB6A"
          },
          {
          "letter": "J",
          "word": "Jackfruit",
          "emoji": "🍈",
          "bgColor": "#F9A825"
          },
          {
          "letter": "K",
          "word": "Kumquat",
          "emoji": "🍊",
          "bgColor": "#FF8F00"
          },
          {
          "letter": "L",
          "word": "Lychee",
          "emoji": "🍒",
          "bgColor": "#E53935"
          },
          {
          "letter": "M",
          "word": "Mango",
          "emoji": "🥭",
          "bgColor": "#FF8F00"
          },
          {
          "letter": "N",
          "word": "Noni",
          "emoji": "🍈",
          "bgColor": "#9E9D24"
          },
          {
          "letter": "O",
          "word": "Olive",
          "emoji": "🫒",
          "bgColor": "#33691E"
          },
          {
          "letter": "P",
          "word": "Papaya",
          "emoji": "🍈",
          "bgColor": "#FF6D00"
          },
          {
          "letter": "Q",
          "word": "Quince",
          "emoji": "🍐",
          "bgColor": "#C0CA33"
          },
          {
          "letter": "R",
          "word": "Rambutan",
          "emoji": "🍒",
          "bgColor": "#C62828"
          },
          {
          "letter": "S",
          "word": "Starfruit",
          "emoji": "⭐",
          "bgColor": "#FFEB3B"
          },
          {
          "letter": "T",
          "word": "Tamarind",
          "emoji": "🌰",
          "bgColor": "#5D4037"
          },
          {
          "letter": "U",
          "word": "Ugli Fruit",
          "emoji": "🍊",
          "bgColor": "#F57F17"
          },
          {
          "letter": "V",
          "word": "Vanilla",
          "emoji": "🌿",
          "bgColor": "#FFF8E1"
          },
          {
          "letter": "W",
          "word": "Watermelon",
          "emoji": "🍉",
          "bgColor": "#43A047"
          },
          {
          "letter": "X",
          "word": "Ximenia",
          "emoji": "🍒",
          "bgColor": "#E65100"
          },
          {
          "letter": "Y",
          "word": "Yellow Plum",
          "emoji": "🍑",
          "bgColor": "#FDD835"
          },
          {
          "letter": "Z",
          "word": "Zucchini",
          "emoji": "🥒",
          "bgColor": "#558B2F"
          },
          {
          "letter": "P",
          "word": "Passion Fruit",
          "emoji": "🍇",
          "bgColor": "#6A1B9A"
          },
          {
          "letter": "P",
          "word": "Pineapple",
          "emoji": "🍍",
          "bgColor": "#F9A825"
          },
          {
          "letter": "K",
          "word": "Kiwi",
          "emoji": "🥝",
          "bgColor": "#689F38"
          },
          {
          "letter": "A",
          "word": "Avocado",
          "emoji": "🥑",
          "bgColor": "#33691E"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcFruitsBerries"
        component={AlphabetShortsTemplate}
        durationInFrames={2880}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "Berry Bonanza",
          videoId: "fruits-berries",
          bgGradient: ["#AD1457","#F48FB1"] as [string, string],
          accentColor: "#880E4F",
          letters: [
          {
          "letter": "A",
          "word": "Acerola",
          "emoji": "🍒",
          "bgColor": "#E53935"
          },
          {
          "letter": "B",
          "word": "Blueberry",
          "emoji": "🫐",
          "bgColor": "#283593"
          },
          {
          "letter": "C",
          "word": "Cranberry",
          "emoji": "🍒",
          "bgColor": "#C62828"
          },
          {
          "letter": "D",
          "word": "Dewberry",
          "emoji": "🫐",
          "bgColor": "#4A148C"
          },
          {
          "letter": "E",
          "word": "Elderberry",
          "emoji": "🫐",
          "bgColor": "#311B92"
          },
          {
          "letter": "F",
          "word": "Feijoa",
          "emoji": "🍈",
          "bgColor": "#558B2F"
          },
          {
          "letter": "G",
          "word": "Gooseberry",
          "emoji": "🍈",
          "bgColor": "#689F38"
          },
          {
          "letter": "H",
          "word": "Huckleberry",
          "emoji": "🫐",
          "bgColor": "#1A237E"
          },
          {
          "letter": "I",
          "word": "Indian Gooseberry",
          "emoji": "🍈",
          "bgColor": "#33691E"
          },
          {
          "letter": "J",
          "word": "Juniper Berry",
          "emoji": "🫐",
          "bgColor": "#004D40"
          },
          {
          "letter": "K",
          "word": "Kiwi Berry",
          "emoji": "🥝",
          "bgColor": "#2E7D32"
          },
          {
          "letter": "L",
          "word": "Lingonberry",
          "emoji": "🍒",
          "bgColor": "#B71C1C"
          },
          {
          "letter": "M",
          "word": "Mulberry",
          "emoji": "🫐",
          "bgColor": "#4A148C"
          },
          {
          "letter": "N",
          "word": "Nectarine",
          "emoji": "🍑",
          "bgColor": "#FF8A65"
          },
          {
          "letter": "O",
          "word": "Olallieberry",
          "emoji": "🫐",
          "bgColor": "#1B0032"
          },
          {
          "letter": "P",
          "word": "Persimmon",
          "emoji": "🍊",
          "bgColor": "#E65100"
          },
          {
          "letter": "Q",
          "word": "Quandong",
          "emoji": "🍒",
          "bgColor": "#D32F2F"
          },
          {
          "letter": "R",
          "word": "Raspberry",
          "emoji": "🍒",
          "bgColor": "#C2185B"
          },
          {
          "letter": "S",
          "word": "Strawberry",
          "emoji": "🍓",
          "bgColor": "#E53935"
          },
          {
          "letter": "T",
          "word": "Tayberry",
          "emoji": "🍇",
          "bgColor": "#880E4F"
          },
          {
          "letter": "U",
          "word": "Uva Ursi",
          "emoji": "🍒",
          "bgColor": "#AD1457"
          },
          {
          "letter": "V",
          "word": "Voavanga",
          "emoji": "🍈",
          "bgColor": "#827717"
          },
          {
          "letter": "W",
          "word": "Wineberry",
          "emoji": "🍇",
          "bgColor": "#880E4F"
          },
          {
          "letter": "X",
          "word": "Ximenia",
          "emoji": "🍒",
          "bgColor": "#BF360C"
          },
          {
          "letter": "Y",
          "word": "Youngberry",
          "emoji": "🫐",
          "bgColor": "#4A148C"
          },
          {
          "letter": "Z",
          "word": "Zante Currant",
          "emoji": "🍇",
          "bgColor": "#311B92"
          },
          {
          "letter": "B",
          "word": "Blackberry",
          "emoji": "🫐",
          "bgColor": "#1A0033"
          },
          {
          "letter": "C",
          "word": "Cherry",
          "emoji": "🍒",
          "bgColor": "#D32F2F"
          },
          {
          "letter": "G",
          "word": "Grape",
          "emoji": "🍇",
          "bgColor": "#6A1B9A"
          },
          {
          "letter": "B",
          "word": "Boysenberry",
          "emoji": "🫐",
          "bgColor": "#4A148C"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcFruitsCitrus"
        component={AlphabetShortsTemplate}
        durationInFrames={2880}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "Citrus & Sour Fruits",
          videoId: "fruits-citrus",
          bgGradient: ["#F9A825","#FFF9C4"] as [string, string],
          accentColor: "#F57F17",
          letters: [
          {
          "letter": "A",
          "word": "Asian Pear",
          "emoji": "🍐",
          "bgColor": "#C0CA33"
          },
          {
          "letter": "B",
          "word": "Blood Orange",
          "emoji": "🍊",
          "bgColor": "#D32F2F"
          },
          {
          "letter": "C",
          "word": "Clementine",
          "emoji": "🍊",
          "bgColor": "#FF8F00"
          },
          {
          "letter": "D",
          "word": "Date",
          "emoji": "🌴",
          "bgColor": "#5D4037"
          },
          {
          "letter": "E",
          "word": "Etrog",
          "emoji": "🍋",
          "bgColor": "#F9A825"
          },
          {
          "letter": "F",
          "word": "Finger Lime",
          "emoji": "🍋",
          "bgColor": "#7CB342"
          },
          {
          "letter": "G",
          "word": "Grapefruit",
          "emoji": "🍊",
          "bgColor": "#FF7043"
          },
          {
          "letter": "H",
          "word": "Hassaku",
          "emoji": "🍊",
          "bgColor": "#FFB300"
          },
          {
          "letter": "I",
          "word": "Iyokan",
          "emoji": "🍊",
          "bgColor": "#FF8F00"
          },
          {
          "letter": "J",
          "word": "Jabara",
          "emoji": "🍋",
          "bgColor": "#AFB42B"
          },
          {
          "letter": "K",
          "word": "Kabosu",
          "emoji": "🍋",
          "bgColor": "#689F38"
          },
          {
          "letter": "L",
          "word": "Lemon",
          "emoji": "🍋",
          "bgColor": "#FDD835"
          },
          {
          "letter": "M",
          "word": "Mandarin",
          "emoji": "🍊",
          "bgColor": "#FF6D00"
          },
          {
          "letter": "N",
          "word": "Navel Orange",
          "emoji": "🍊",
          "bgColor": "#EF6C00"
          },
          {
          "letter": "O",
          "word": "Orange",
          "emoji": "🍊",
          "bgColor": "#FF9800"
          },
          {
          "letter": "P",
          "word": "Pomelo",
          "emoji": "🍊",
          "bgColor": "#C0CA33"
          },
          {
          "letter": "Q",
          "word": "Quince",
          "emoji": "🍐",
          "bgColor": "#CDDC39"
          },
          {
          "letter": "R",
          "word": "Rangpur",
          "emoji": "🍊",
          "bgColor": "#FF6F00"
          },
          {
          "letter": "S",
          "word": "Satsuma",
          "emoji": "🍊",
          "bgColor": "#FF8F00"
          },
          {
          "letter": "T",
          "word": "Tangerine",
          "emoji": "🍊",
          "bgColor": "#E65100"
          },
          {
          "letter": "U",
          "word": "Ugly Fruit",
          "emoji": "🍊",
          "bgColor": "#9E9D24"
          },
          {
          "letter": "V",
          "word": "Valencia Orange",
          "emoji": "🍊",
          "bgColor": "#FF9800"
          },
          {
          "letter": "W",
          "word": "White Grapefruit",
          "emoji": "🍊",
          "bgColor": "#FFF9C4"
          },
          {
          "letter": "X",
          "word": "Xiangcheng",
          "emoji": "🍊",
          "bgColor": "#F57F17"
          },
          {
          "letter": "Y",
          "word": "Yuzu",
          "emoji": "🍋",
          "bgColor": "#FDD835"
          },
          {
          "letter": "Z",
          "word": "Zabon",
          "emoji": "🍊",
          "bgColor": "#AFB42B"
          },
          {
          "letter": "L",
          "word": "Lime",
          "emoji": "🍋",
          "bgColor": "#558B2F"
          },
          {
          "letter": "K",
          "word": "Kumquat",
          "emoji": "🍊",
          "bgColor": "#FF8F00"
          },
          {
          "letter": "C",
          "word": "Calamansi",
          "emoji": "🍋",
          "bgColor": "#689F38"
          },
          {
          "letter": "B",
          "word": "Bergamot",
          "emoji": "🍋",
          "bgColor": "#9E9D24"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcFruitsGarden"
        component={AlphabetShortsTemplate}
        durationInFrames={2880}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "Garden Fruits",
          videoId: "fruits-garden",
          bgGradient: ["#43A047","#C8E6C9"] as [string, string],
          accentColor: "#2E7D32",
          letters: [
          {
          "letter": "A",
          "word": "Apple",
          "emoji": "🍎",
          "bgColor": "#E53935"
          },
          {
          "letter": "B",
          "word": "Blackcurrant",
          "emoji": "🫐",
          "bgColor": "#1A237E"
          },
          {
          "letter": "C",
          "word": "Cherry",
          "emoji": "🍒",
          "bgColor": "#C62828"
          },
          {
          "letter": "D",
          "word": "Damson",
          "emoji": "🍇",
          "bgColor": "#4A148C"
          },
          {
          "letter": "E",
          "word": "Elderberry",
          "emoji": "🫐",
          "bgColor": "#311B92"
          },
          {
          "letter": "F",
          "word": "Fig",
          "emoji": "🍇",
          "bgColor": "#6A1B9A"
          },
          {
          "letter": "G",
          "word": "Greengage",
          "emoji": "🍈",
          "bgColor": "#689F38"
          },
          {
          "letter": "H",
          "word": "Hazelnut",
          "emoji": "🌰",
          "bgColor": "#6D4C41"
          },
          {
          "letter": "I",
          "word": "Indian Jujube",
          "emoji": "🍈",
          "bgColor": "#827717"
          },
          {
          "letter": "J",
          "word": "Jujube",
          "emoji": "🍒",
          "bgColor": "#BF360C"
          },
          {
          "letter": "K",
          "word": "Kiwi",
          "emoji": "🥝",
          "bgColor": "#558B2F"
          },
          {
          "letter": "L",
          "word": "Lemon",
          "emoji": "🍋",
          "bgColor": "#FDD835"
          },
          {
          "letter": "M",
          "word": "Melon",
          "emoji": "🍈",
          "bgColor": "#C5E1A5"
          },
          {
          "letter": "N",
          "word": "Nectarine",
          "emoji": "🍑",
          "bgColor": "#FF8A65"
          },
          {
          "letter": "O",
          "word": "Orange",
          "emoji": "🍊",
          "bgColor": "#FF9800"
          },
          {
          "letter": "P",
          "word": "Peach",
          "emoji": "🍑",
          "bgColor": "#FFAB91"
          },
          {
          "letter": "Q",
          "word": "Quince",
          "emoji": "🍐",
          "bgColor": "#CDDC39"
          },
          {
          "letter": "R",
          "word": "Redcurrant",
          "emoji": "🍒",
          "bgColor": "#D32F2F"
          },
          {
          "letter": "S",
          "word": "Strawberry",
          "emoji": "🍓",
          "bgColor": "#E53935"
          },
          {
          "letter": "T",
          "word": "Tomato",
          "emoji": "🍅",
          "bgColor": "#F44336"
          },
          {
          "letter": "U",
          "word": "Ugli",
          "emoji": "🍊",
          "bgColor": "#9E9D24"
          },
          {
          "letter": "V",
          "word": "Victoria Plum",
          "emoji": "🍇",
          "bgColor": "#7B1FA2"
          },
          {
          "letter": "W",
          "word": "Walnut",
          "emoji": "🌰",
          "bgColor": "#5D4037"
          },
          {
          "letter": "X",
          "word": "Ximenia",
          "emoji": "🍒",
          "bgColor": "#E65100"
          },
          {
          "letter": "Y",
          "word": "Yellow Apple",
          "emoji": "🍏",
          "bgColor": "#CDDC39"
          },
          {
          "letter": "Z",
          "word": "Zucchini",
          "emoji": "🥒",
          "bgColor": "#33691E"
          },
          {
          "letter": "P",
          "word": "Pear",
          "emoji": "🍐",
          "bgColor": "#C0CA33"
          },
          {
          "letter": "P",
          "word": "Plum",
          "emoji": "🍇",
          "bgColor": "#6A1B9A"
          },
          {
          "letter": "A",
          "word": "Apricot",
          "emoji": "🍑",
          "bgColor": "#FFB74D"
          },
          {
          "letter": "G",
          "word": "Grape",
          "emoji": "🍇",
          "bgColor": "#4A148C"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
      <Composition
        id="AbcFruitsExotic"
        component={AlphabetShortsTemplate}
        durationInFrames={2880}
        fps={30}
        width={1080}
        height={1920}
        defaultProps={{
          title: "Exotic Fruits",
          videoId: "fruits-exotic",
          bgGradient: ["#6A1B9A","#E1BEE7"] as [string, string],
          accentColor: "#4A148C",
          letters: [
          {
          "letter": "A",
          "word": "Ackee",
          "emoji": "🍈",
          "bgColor": "#F9A825"
          },
          {
          "letter": "B",
          "word": "Buddha's Hand",
          "emoji": "🍋",
          "bgColor": "#FDD835"
          },
          {
          "letter": "C",
          "word": "Cherimoya",
          "emoji": "🍈",
          "bgColor": "#689F38"
          },
          {
          "letter": "D",
          "word": "Durian",
          "emoji": "🍈",
          "bgColor": "#827717"
          },
          {
          "letter": "E",
          "word": "Ensete",
          "emoji": "🍌",
          "bgColor": "#558B2F"
          },
          {
          "letter": "F",
          "word": "Feijoa",
          "emoji": "🍈",
          "bgColor": "#33691E"
          },
          {
          "letter": "G",
          "word": "Gac Fruit",
          "emoji": "🍈",
          "bgColor": "#E65100"
          },
          {
          "letter": "H",
          "word": "Horned Melon",
          "emoji": "🍈",
          "bgColor": "#FF8F00"
          },
          {
          "letter": "I",
          "word": "Ice Cream Bean",
          "emoji": "🍦",
          "bgColor": "#4CAF50"
          },
          {
          "letter": "J",
          "word": "Jabuticaba",
          "emoji": "🫐",
          "bgColor": "#311B92"
          },
          {
          "letter": "K",
          "word": "Kiwano",
          "emoji": "🍈",
          "bgColor": "#FF6D00"
          },
          {
          "letter": "L",
          "word": "Longan",
          "emoji": "🍈",
          "bgColor": "#8D6E63"
          },
          {
          "letter": "M",
          "word": "Mangosteen",
          "emoji": "🍈",
          "bgColor": "#6A1B9A"
          },
          {
          "letter": "N",
          "word": "Naranjilla",
          "emoji": "🍊",
          "bgColor": "#FF8F00"
          },
          {
          "letter": "O",
          "word": "Osage Orange",
          "emoji": "🍈",
          "bgColor": "#9E9D24"
          },
          {
          "letter": "P",
          "word": "Pitaya",
          "emoji": "🐉",
          "bgColor": "#E91E63"
          },
          {
          "letter": "Q",
          "word": "Quenepa",
          "emoji": "🍈",
          "bgColor": "#2E7D32"
          },
          {
          "letter": "R",
          "word": "Rollinia",
          "emoji": "🍈",
          "bgColor": "#F9A825"
          },
          {
          "letter": "S",
          "word": "Sapodilla",
          "emoji": "🍈",
          "bgColor": "#6D4C41"
          },
          {
          "letter": "T",
          "word": "Tamarillo",
          "emoji": "🍅",
          "bgColor": "#D32F2F"
          },
          {
          "letter": "U",
          "word": "Uvaia",
          "emoji": "🍈",
          "bgColor": "#FFD600"
          },
          {
          "letter": "V",
          "word": "Velvet Apple",
          "emoji": "🍎",
          "bgColor": "#BF360C"
          },
          {
          "letter": "W",
          "word": "Wax Apple",
          "emoji": "🍎",
          "bgColor": "#C62828"
          },
          {
          "letter": "X",
          "word": "Ximenia",
          "emoji": "🍒",
          "bgColor": "#E65100"
          },
          {
          "letter": "Y",
          "word": "Yellow Mangosteen",
          "emoji": "🍈",
          "bgColor": "#F57F17"
          },
          {
          "letter": "Z",
          "word": "Zapote",
          "emoji": "🍈",
          "bgColor": "#4E342E"
          },
          {
          "letter": "S",
          "word": "Snake Fruit",
          "emoji": "🍈",
          "bgColor": "#795548"
          },
          {
          "letter": "S",
          "word": "Star Apple",
          "emoji": "⭐",
          "bgColor": "#7B1FA2"
          },
          {
          "letter": "S",
          "word": "Soursop",
          "emoji": "🍈",
          "bgColor": "#2E7D32"
          },
          {
          "letter": "C",
          "word": "Cempedak",
          "emoji": "🍈",
          "bgColor": "#FF8F00"
          }
          ],
          letterDuration: 3,
          introDuration: 3,
          outroDuration: 3,
        }}
      />
    </>
  );
};

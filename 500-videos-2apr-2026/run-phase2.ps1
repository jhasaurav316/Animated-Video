# ============================================================================
# Phase 2 - 50 Videos (10 Professions + 30 Famous Places + 10 Dog Breeds)
# ============================================================================
# Batch: 500-videos-2apr-2026
# Usage: cd C:\test\500-videos-2apr-2026 && .\run-phase2.ps1
# ============================================================================
...
Write-Host "  PHASE 2: 10 Professions + 30 Famous Places + 10 Dog Breeds" -ForegroundColor Cyan
...
$catalogs = @(
    @{ Path = "professions\catalog.json"; Name = "Professions"; From = 20; To = 30 },
    @{ Path = "famous-places\catalog.json"; Name = "Famous Places"; From = 0; To = 30 },
    @{ Path = "dog-breeds\catalog.json"; Name = "Dog Breeds"; From = 0; To = 10 }
)
...
node (Join-Path $ProjectDir "professions\register-compositions.js")
node (Join-Path $ProjectDir "famous-places\register-compositions.js")
node (Join-Path $ProjectDir "dog-breeds\register-compositions.js")
...
foreach ($cat in $catalogs) {
    ...
    foreach ($video in $subset) {
        ...
        $prevEAP = $ErrorActionPreference; $ErrorActionPreference = "Continue"
        npx remotion render $compId "$outputFile" --concurrency=100% --log=error --crf=18 --codec=h264 --gl=angle --enable-multiprocess-on-linux
        $ErrorActionPreference = $prevEAP
